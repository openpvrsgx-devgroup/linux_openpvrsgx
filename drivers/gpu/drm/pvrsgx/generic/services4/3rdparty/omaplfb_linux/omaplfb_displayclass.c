...
static inline void OMAPLFBSetDevInfoPtr(unsigned uiFBDevID,
...
static inline OMAPLFB_BOOL SwapChainHasChanged(OMAPLFB_DEVINFO *psDevInfo,
...
static inline OMAPLFB_BOOL DontWaitForVSync(OMAPLFB_DEVINFO *psDevInfo)
{
	...
	bDontWait = bDontWait ||
	    OMAPLFBAtomicBoolRead(&psDevInfo->sEarlySuspendFlag);
#endif
...
static PVRSRV_ERROR OpenDCDevice(IMG_UINT32 uiPVRDevID, IMG_HANDLE *phDevice,
	...
	IMG_BOOL *pbIsContiguous, IMG_UINT32 *pui32TilingStride)
{
	...
	*pui32ByteSize = (IMG_UINT32)psDevInfo->sFBInfo.ulBufferSize;
	...
	if (ppvCpuVAddr) {
	*ppvCpuVAddr = psDevInfo->sFBInfo.bIs2D ?
	       NULL :
	       psSystemBuffer->sCPUVAddr;
	}
	...
	*pbIsContiguous = !psDevInfo->sFBInfo.bIs2D;
	...
	int i = (psSystemBuffer->sSysAddr.uiAddr -
	 psDevInfo->sFBInfo.psPageList->uiAddr) >>
	PAGE_SHIFT;
	*ppsSysAddr = psDevInfo->sFBInfo.psPageList +
	      psDevInfo->sFBInfo.ulHeight * i;
	}
#endif
	      ...
	      IMG_UINT32 *pui32SwapChainID)
{
	...
	printk(KERN_WARNING DRIVER_PREFIX
	       ": %s: Device %u: Couldn't set frame buffer update mode %d\n",
	...
	psBuffer[i].ulYOffset =
	ui32BufferOffset / psDevInfo->sFBInfo.ulByteStride;
	if (psDevInfo->sFBInfo.bIs2D) {
	psBuffer[i].sSysAddr.uiAddr +=
	ui32SwapBuffer *
	ALIGN((IMG_UINT32)psDevInfo->sFBInfo.ulWidth *
	      psDevInfo->sFBInfo.uiBytesPerPixel,
	      PAGE_SIZE);
	}
...
void OMAPLFBGetBltFBsBvHndl(OMAPLFB_FBINFO *psPVRFBInfo,
	    IMG_UINTPTR_T *ppPhysAddr)
{
	*ppPhysAddr = 0;
}
...
void OMAPLFBSwapHandler(OMAPLFB_BUFFER *psBuffer)
{
	...
	int iBlankEvents =
	OMAPLFBAtomicIntRead(&psDevInfo->sBlankEvents);
	...
	psSwapChain->iBlankEvents = iBlankEvents;
	psSwapChain->bNotVSynced =
	!WaitForVSyncSettle(psDevInfo);
	...
	(IMG_HANDLE)psBuffer->hCmdComplete, IMG_TRUE);
}
...
static IMG_BOOL ProcessFlipV1(IMG_HANDLE hCmdCookie, OMAPLFB_DEVINFO *psDevInfo,
	      OMAPLFB_SWAPCHAIN *psSwapChain,
	      OMAPLFB_BUFFER *psBuffer,
	      unsigned long ulSwapInterval)
{
	...
	IMG_UINT32 w =
	psBuffer->psDevInfo->sDisplayDim.ui32Width;
	IMG_UINT32 h =
	psBuffer->psDevInfo->sDisplayDim.ui32Height;
...
int meminfo_idx_valid(unsigned int meminfo_ix, int num_meminfos)
{
	...
	WARN(1, "%s: Invalid meminfo index %d, max %d\n", __func__,
	     meminfo_ix, num_meminfos);
	return 0;
	}
	return 1;
}

static IMG_BOOL ProcessFlipV2(IMG_HANDLE hCmdCookie, OMAPLFB_DEVINFO *psDevInfo,
	      ...
	      IMG_UINT32 uiHwcDataSz)
{
	...
	calcsz =
	sizeof(*psHwcData) + (sizeof(struct rgz_blt_entry) * rgz_items);
	iUseBltFB = psHwcData->blit_data.rgz_flags & HWC_BLT_FLAG_USE_FB;

	if (!iUseBltFB && rgz_items > 0) {
	WARN(1,
	     "Trying to blit without a pipe configured for the blit FB");
	return IMG_FALSE;
	}
	...
	WARN(1, "invalid size of private data (%d vs %d)", uiHwcDataSz,
	     calcsz);
	}
	...
	OMAPLFBGetBltFBsBvHndl(&psDevInfo->sFBInfo,
	       &asMemInfo[0].uiAddr);
	}

	for (i = 0, k = iMemIdx;
	     i < ui32NumMemInfos && k < ARRAY_SIZE(apsTilerPAs) &&
	     k < psDssData->num_ovls;
	     i++, k++) {
	struct tiler_pa_info *psTilerInfo;
	...
	psDevInfo->sPVRJTable.pfnPVRSRVDCMemInfoGetByteSize(
	ppsMemInfos[i], &uByteSize);
	ui32NumPages = (uByteSize + PAGE_SIZE - 1) >> PAGE_SHIFT;

	psDevInfo->sPVRJTable.pfnPVRSRVDCMemInfoGetCpuPAddr(
	ppsMemInfos[i], 0, &phyAddr);

#ifdef CONFIG_DSSCOMP_ADAPT
	{
	OMAPLFB_FBINFO *psPVRFBInfo = &psDevInfo->sFBInfo;
	if (i == k //no Blit
	    && psPVRFBInfo->sSysAddr.uiAddr <= phyAddr.uiAddr &&
	    phyAddr.uiAddr < (psPVRFBInfo->sSysAddr.uiAddr +
	      psPVRFBInfo->ulFBSize)) {
	//[LGE_UPDATE_S] OSP fb0 read issue Fixed jeonghoon.cho@lge.com 0415
	struct fb_info *fb = psDevInfo->psLINFBInfo;
	//[LGE_UPDATE_E] OSP fb0 read issue Fixed jeonghoon.cho@lge.com
	...
	if (fb != NULL) {
	unsigned offset =
	(unsigned)psDssData->ovls[i].ba;
	unsigned line_lenght =
	fb->fix.line_length;
	unsigned bpp =
	fb->var.bits_per_pixel / 8;

	//change offset to give information to fb capturing apps.
	fb->var.yoffset = offset / line_lenght;
	fb->var.xoffset =
	(offset % line_lenght) / bpp;
	}
	//[LGE_UPDATE_E] OSP fb0 read issue Fixed jeonghoon.cho@lge.com
	continue;
	}
	...
	asMemInfo[k].uiAddr = phyAddr.uiAddr +
	      ppsMemInfos[i]->planeOffsets[0];
	if (tiler_fmt((u32)phyAddr.uiAddr) == TILFMT_8BIT) {
	IMG_UINT32 uvOffset =
	ppsMemInfos[i]->planeOffsets[1] ?
	ppsMemInfos[i]->planeOffsets[1] :
	((uByteSize * 2) / 3);
	psDevInfo->sPVRJTable
	.pfnPVRSRVDCMemInfoGetCpuPAddr(
	ppsMemInfos[i], uvOffset,
	&phyAddr);
	...
	if (phyAddr.uiAddr >= psDevInfo->psLINFBInfo->fix.smem_start &&
	    phyAddr.uiAddr < (psDevInfo->psLINFBInfo->fix.smem_start +
	      psDevInfo->psLINFBInfo->fix.smem_len)) {
	asMemInfo[k].uiAddr = phyAddr.uiAddr;
	continue;
	}
	...
	psTilerInfo->mem = kzalloc(
	sizeof(*psTilerInfo->mem) * ui32NumPages, GFP_KERNEL);
	if (!psTilerInfo->mem) {
	kfree(psTilerInfo);
	continue;
	}
	...
	psDevInfo->sPVRJTable.pfnPVRSRVDCMemInfoGetCpuVAddr(
	ppsMemInfos[i], &virtAddr);
	...
	if (psDssData->ovls[i].addressing !=
	...
	if (rgz_items > 0) {
	OMAPLFBDoBlits(psDevInfo, ppsMemInfos, &psHwcData->blit_data,
	       ui32NumMemInfos);
	}

	dsscomp_gralloc_queue(psDssData, apsTilerPAs, false,
	      dsscomp_proxy_cmdcomplete, (void *)hCmdCookie);
	    ...
	    IMG_VOID *pvData)
{
	...
	return ProcessFlipV1(hCmdCookie, psDevInfo,
	...
	return ProcessFlipV2(hCmdCookie, psDevInfo,
	...
	OMAPLFB_FBINFO *psPVRFBInfo)
{
	...
	.w = ALIGN(psLINFBInfo->var.xres,
	   PAGE_SIZE / (psLINFBInfo->var.bits_per_pixel / 8)),
	.h = psLINFBInfo->var.yres,
	.fmt = psLINFBInfo->var.bits_per_pixel == 16 ?
	       TILER_PIXEL_FMT_16BIT :
	       TILER_PIXEL_FMT_32BIT,
	.flags = 0,
	.token = 0,
	.out_align = PAGE_SIZE
	};
	...
	printk(KERN_WARNING DRIVER_PREFIX
	       ": %s: Device %u: Swap chain length missing in "
	       "platform data, defaulting to %d\n",
	       __FUNCTION__, psDevInfo->uiFBDevID, iMaxSwapChainBuffs);
	} else {
	...
	printk(KERN_WARNING DRIVER_PREFIX
	       ": %s: Device %u: Trying to use %d tiler "
	       "buffers which is less than the swap chain length of %d, maximum "
	       "swap chain length will be set to %d\n",
	       __FUNCTION__, psDevInfo->uiFBDevID,
	       psFBPlatConfig->tiler2d_buffers, iMaxSwapChainBuffs,
	       psFBPlatConfig->tiler2d_buffers);
	...
	psPVRFBInfo->ulByteStride =
	PAGE_ALIGN(psPVRFBInfo->ulWidth * psPVRFBInfo->uiBytesPerPixel);
	...
	kzalloc(w * n * psPVRFBInfo->ulHeight *
	sizeof(*psPVRFBInfo->psPageList),
	GFP_KERNEL);
	if (!psPVRFBInfo->psPageList) {
	printk(KERN_WARNING DRIVER_PREFIX
	       ": %s: Device %u: Could not allocate page list\n",
	       __FUNCTION__, psDevInfo->uiFBDevID);
	...
	tilview_create(&view, phys, psDevInfo->sFBInfo.ulWidth,
	       psDevInfo->sFBInfo.ulHeight);
	for (i = 0; i < n; i++) {
	       ...
	       OMAPLFB_FBINFO *psPVRFBInfo)
{
	...
	GetFBPlatConfig(psDevInfo->uiFBDevID);
	...
	printk(KERN_WARNING DRIVER_PREFIX
	       ": %s: Device %u: Trying to use %d vram "
	       "buffers which is less than the swap chain length of %d, maximum "
	       "swap chain length will be set to %d\n",
	       __FUNCTION__, psDevInfo->uiFBDevID,
	       psFBPlatConfig->vram_buffers, iMaxSwapChainBuffs,
	       psFBPlatConfig->vram_buffers);
	...
	RoundUpToMultiple(psPVRFBInfo->ulBufferSize, ulLCM);
	ui32FBAvailableBuffs =
	(IMG_UINT32)(psDevInfo->sFBInfo.ulFBSize /
	     psDevInfo->sFBInfo.ulRoundedBufferSize);

	if (!ui32FBAvailableBuffs) {
	printk(KERN_ERR DRIVER_PREFIX
	       " %s: Device %u: Not enough vram to init swap "
	       "chain buffers\n",
	       __FUNCTION__, psDevInfo->uiFBDevID);
	return OMAPLFB_ERROR_INIT_FAILURE;
	} else if (ui32FBAvailableBuffs < psFBPlatConfig->vram_buffers) {
	printk(KERN_WARNING DRIVER_PREFIX
	       ": %s: Device %u: Not enough vram to hold "
	       "%d buffers (available %d), swap chain length will be set to %d\n",
	       __FUNCTION__, psDevInfo->uiFBDevID, iMaxSwapChainBuffs,
	       ui32FBAvailableBuffs, ui32FBAvailableBuffs);
...
static OMAPLFB_ERROR OMAPLFBInitFBDev(OMAPLFB_DEVINFO *psDevInfo)
{
	...
	eError =
	OMAPLFBInitIonOmap(psDevInfo, psLINFBInfo, psPVRFBInfo);
	if (eError != OMAPLFB_OK) {
	goto ErrorModPut;
	}
	} else
	...
	((int)psLINFBInfo->var.height > 0) ? psLINFBInfo->var.height :
	     54;
...
static OMAPLFB_DEVINFO *OMAPLFBInitDev(unsigned uiFBDevID)
{
	...
	psDevInfo = (OMAPLFB_DEVINFO *)OMAPLFBAllocKernelMem(
	sizeof(OMAPLFB_DEVINFO));
	...
	if (psDevInfo->sPVRJTable.pfnPVRSRVRegisterDCDevice(
	    &psDevInfo->sDCJTable, &psDevInfo->uiPVRDevID) !=
	    PVRSRV_OK) {
	printk(KERN_ERR DRIVER_PREFIX
	...
	if (psDevInfo->sPVRJTable.pfnPVRSRVRegisterCmdProcList(
	    psDevInfo->uiPVRDevID, &pfnCmdProcList[0],
...
ErrorUnregisterDevice:
	(void)psDevInfo->sPVRJTable.pfnPVRSRVRemoveDCDevice(
	psDevInfo->uiPVRDevID);
ErrorDeInitFBDev:
...
static OMAPLFB_BOOL OMAPLFBDeInitDev(OMAPLFB_DEVINFO *psDevInfo)
{
	...
	if (psPVRJTable->pfnPVRSRVRemoveCmdProcList(psDevInfo->uiPVRDevID,
	...
	if (psPVRJTable->pfnPVRSRVRemoveDCDevice(psDevInfo->uiPVRDevID) !=
	    PVRSRV_OK) {
	printk(KERN_ERR DRIVER_PREFIX
