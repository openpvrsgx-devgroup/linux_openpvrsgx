		    ...
		    int stream)
{
	...
	dev_dbg(be->dev, "%s: %s %d\n", __func__, snd_soc_rtd_to_cpu(be, 0)->name, stream);
				 ...
				 struct snd_soc_dai *dai, int mute)
{
	...
	switch (snd_soc_rtd_to_cpu(fe, 0)->id) {
	case OMAP_AESS_DAI_MM2_CAPTURE:
		if (mute) {
			    ...
			    struct snd_soc_dai *dai, int cmd)
{
	...
	dev_dbg(fe->dev, "%s: %s %d\n", __func__, snd_soc_rtd_to_cpu(fe, 0)->name, stream);
	...
	case SNDRV_PCM_TRIGGER_START:
		...
		list_for_each_entry(dpcm, be_clients, list_be) {
			...
			/* trigger the BE port */
			snd_soc_pcm_dai_trigger(be_substream, cmd, false);
		...
		if (snd_soc_dpcm_fe_can_update(fe, stream)) {
			...
			snd_soc_pcm_dai_trigger(substream, cmd, false);
