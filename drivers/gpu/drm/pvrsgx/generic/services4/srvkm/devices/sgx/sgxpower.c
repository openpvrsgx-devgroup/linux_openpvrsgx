...
static PVRSRV_ERROR SGXUpdateTimingInfo(PVRSRV_DEVICE_NODE *psDeviceNode)
{
	...
	eError = SGXAddTimer(psDeviceNode, psSGXTimingInfo,
	     &psDevInfo->hTimer);
	if (eError != PVRSRV_OK) {
	return eError;
	}
	...
	psDevInfo->ui32uKernelTimerClock = psSGXTimingInfo->ui32CoreClockSpeed /
	   psSGXTimingInfo->ui32uKernelFreq;
	      ...
	      IMG_CHAR *pszComment)
{
	...
	if (PollForValueKM(
	    (IMG_UINT32 *)psDevInfo->pvRegsBaseKM + (ui32Register >> 2),
	    0, ui32RegisterValue, MAX_HW_TIME_US,
	    MAX_HW_TIME_US / WAIT_TRY_COUNT, IMG_FALSE) != PVRSRV_OK) {
	      ...
	      PVRSRV_DEV_POWER_STATE eCurrentPowerState)
{
	...
	eError = SGXScheduleCCBCommand(psDeviceNode, SGXMKIF_CMD_POWER,
	...
	PVR_DPF((
	PVR_DBG_ERROR,
	"SGXPrePowerState: SGXDeinitialise failed: %u",
	eError));
	return eError;
	}
	}
	       ...
	       PVRSRV_DEV_POWER_STATE eCurrentPowerState)
{
	...
	PVR_DPF((
	PVR_DBG_ERROR,
	"SGXPostPowerState failed to schedule CCB command: %u",
	eError));
	return eError;
	}
	}
	    ...
	    PVRSRV_DEV_POWER_STATE eCurrentPowerState)
{
	...
	eError = SGXPrePowerState(hDevHandle,
	...
	PVR_DPF((PVR_DBG_MESSAGE,
	 "SGXPreClockSpeedChange: SGX clock speed was %uHz",
	     ...
	     PVRSRV_DEV_POWER_STATE eCurrentPowerState)
{
	...
	if (bIdleDevice) {
	eError = SGXPostPowerState(hDevHandle,
	   PVRSRV_DEV_POWER_STATE_ON,
	   PVRSRV_DEV_POWER_STATE_IDLE);
	...
	PVR_DPF((
	PVR_DBG_MESSAGE,
	"SGXPostClockSpeedChange: SGX clock speed changed from %uHz to %uHz",
