...
MRST_ERROR MRSTLFBGetLibFuncAddr(char *szFunctionName,
...
static void MRSTLFBVSyncWriteReg(MRSTLFB_DEVINFO *psDevInfo,
	 unsigned long ulOffset, unsigned long ulValue)
{
...
unsigned long MRSTLFBVSyncReadReg(MRSTLFB_DEVINFO *psDevinfo,
...
void MRSTLFBDisableVSyncInterrupt(MRSTLFB_DEVINFO *psDevinfo)
{
	...
	psb_disable_vblank(psDevinfo->psDrmDevice,
	   dev_priv->ui32MainPipe);
#endif
}
...
int PVR_DRM_MAKENAME(DISPLAY_CONTROLLER, _Init)(struct drm_device unref__ *dev)
{
	...
	printk(KERN_WARNING DRIVER_PREFIX
	       ": MRSTLFB_Init: MRSTLFBInit failed\n");
	return -ENODEV;
	}
