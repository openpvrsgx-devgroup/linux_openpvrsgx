...
static PVRSRV_ERROR PCIInitDev(SYS_DATA *psSysData)
{
	...
	PVR_DPF((PVR_DBG_ERROR,
	 "PCIInitDev: Failed to get (0,0,0) device"));
	return PVRSRV_ERROR_NOT_SUPPORTED;
	}
...
static PVRSRV_ERROR SysLocateDevices(SYS_DATA *psSysData)
{
	...
	ui32BaseAddr +
	(IS_CDV(gpDrmDevice) ? SGX_REGS_OFFSET : PSB_SGX_REGS_OFFSET);
	...
	gsSGXDeviceMap.sSPCpuPBase =
	SysSysPAddrToCpuPAddr(gsSGXDeviceMap.sSPSysPBase);
...
PVRSRV_ERROR SysInitialise(IMG_VOID)
{
	...
	if (eError != PVRSRV_OK) {
	PVR_DPF((PVR_DBG_ERROR,
	 "SysInitialise: Failed to register MSVDXdevice!"));
	SysDeinitialise(gpsSysData);
	gpsSysData = IMG_NULL;
	return eError;
	}
...
PVRSRV_ERROR SysSystemPrePowerState(PVRSRV_SYS_POWER_STATE eNewPowerState)
{
	...
	pci_read_config_dword(psPVRPCI->psPCIDev, 0x5C,
	      &gsSysSpecificData.saveBSM);
	pci_read_config_dword(psPVRPCI->psPCIDev, 0xFC,
	      &gsSysSpecificData.saveVBT);
	pci_read_config_dword(psPVRPCI->psPCIDev,
	      MRST_PCIx_MSI_ADDR_LOC,
	      &gsSysSpecificData.msi_addr);
	pci_read_config_dword(psPVRPCI->psPCIDev,
	      MRST_PCIx_MSI_DATA_LOC,
	      &gsSysSpecificData.msi_data);
	}
	}
...
PVRSRV_ERROR SysSystemPostPowerState(PVRSRV_SYS_POWER_STATE eNewPowerState)
{
	...
	pci_write_config_dword(psPVRPCI->psPCIDev, 0x5c,
	       gsSysSpecificData.saveBSM);
	pci_write_config_dword(psPVRPCI->psPCIDev, 0xFC,
	       gsSysSpecificData.saveVBT);
	pci_write_config_dword(psPVRPCI->psPCIDev,
	       MRST_PCIx_MSI_ADDR_LOC,
	       gsSysSpecificData.msi_addr);
	pci_write_config_dword(psPVRPCI->psPCIDev,
	       MRST_PCIx_MSI_DATA_LOC,
	     ...
	     PVRSRV_DEV_POWER_STATE eCurrentPowerState)
{
	...
	PVR_DPF((PVR_DBG_MESSAGE,
	 "SysDevicePrePowerState: Restore MSVDX power"));
	...
	PVR_DPF((PVR_DBG_MESSAGE,
	 "SysDevicePrePowerState: Restore TOPAZ power"));
