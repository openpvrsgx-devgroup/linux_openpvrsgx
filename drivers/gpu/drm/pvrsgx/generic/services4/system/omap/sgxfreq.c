...
sgxfreq_gov_init_t *sgxfreq_gov_init[] = {
	onoff_init, activeidle_init, on3demand_init, userspace_init, NULL,
};

typedef int sgxfreq_gov_deinit_t(void);
sgxfreq_gov_deinit_t *sgxfreq_gov_deinit[] = {
	onoff_deinit, activeidle_deinit, on3demand_deinit, userspace_deinit,
	NULL,
};
...
static ssize_t show_governor(struct device *dev, struct device_attribute *attr,
	     char *buf)
{
	if (sfd.gov)
...
static void __update_timing_info(bool active)
{
	...
	SGXFREQ_TRACE(
	"A->A TA:= %lums \tdA: %lums \tTI: %lums \tdI: %lums\n",
	sfd.total_active_time,
	__delta32(_active_curr_time, _active_prev_time),
	sfd.total_active_time, (unsigned long)0);
	...
	SGXFREQ_TRACE(
	"I->A TA:= %lums \tdA: %lums \tTI: %lums \tdI: %lums\n",
	sfd.total_active_time, (unsigned long)0,
	sfd.total_idle_time,
	__delta32(_idle_curr_time, _idle_prev_time));
	}
	...
	__delta32(_active_curr_time, _active_prev_time);
	SGXFREQ_TRACE(
	"A->I TA:= %lums \tdA: %lums \tTI: %lums \tdI: %lums\n",
	sfd.total_active_time,
	__delta32(_active_curr_time, _active_prev_time),
	sfd.total_active_time, (unsigned long)0);
	} else {
	...
	__delta32(_idle_curr_time, _idle_prev_time);
	SGXFREQ_TRACE(
	"I->I TA:= %lums \tdA: %lums \tTI: %lums \tdI: %lums\n",
	sfd.total_active_time, (unsigned long)0,
	sfd.total_idle_time,
	__delta32(_idle_curr_time, _idle_prev_time));
...
int sgxfreq_init(struct device *dev)
{
	...
	if (!sfd.pdata || !sfd.pdata->opp_get_opp_count ||
	    !sfd.pdata->opp_find_freq_ceil || !sfd.pdata->device_scale)
	return -EINVAL;
#endif
	...
	sfd.clk_nb = of_pm_voltdm_notifier_register(dev, np, sfd.gpu_core_clk,
	    "gpu", &voltage_latency);
	...
	if (ret == -EPROBE_DEFER) {
	dev_err(dev, "gpu clock notifier not ready, retry\n");
	} else {
	dev_err(dev,
...
#endif
	sfd.freq_list =
	kmalloc(sfd.freq_cnt * sizeof(unsigned long), GFP_ATOMIC);
	if (!sfd.freq_list) {
	rcu_read_unlock();
	return -ENOMEM;
	}
