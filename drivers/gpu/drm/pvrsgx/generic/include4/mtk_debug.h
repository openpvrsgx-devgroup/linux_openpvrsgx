...
enum MTK_PP_REGISTER { MTK_PP_R_SGXOSTimer, MTK_PP_R_DEVMEM, MTK_PP_R_SIZE };

#ifdef MTK_DEBUG

IMG_IMPORT IMG_VOID IMG_CALLCONV MTKDebugInit(IMG_VOID);
IMG_IMPORT IMG_VOID IMG_CALLCONV MTKDebugDeinit(IMG_VOID);
IMG_IMPORT IMG_VOID IMG_CALLCONV MTKDebugSetInfo(const IMG_CHAR *acDebugMsg,
	 IMG_INT32 i32Size);
IMG_IMPORT IMG_VOID IMG_CALLCONV MTKDebugGetInfo(IMG_CHAR *acDebugMsg,
	 IMG_INT32 i32Size);
...
enum MTK_PP_BUFFER { MTK_PP_B_QUEUEBUFFER, MTK_PP_B_RINGBUFFER };
...
typedef struct MTK_PROC_PRINT_DATA_TAG {
	...
	void (*pfn_print)(struct MTK_PROC_PRINT_DATA_TAG *data, const char *fmt,
...
#define _PVR_DPF_MTKPP(level, ...)                                      \
	do {                                                            \
	MTK_PROC_PRINT_DATA *mtkpp_data =                       \
	MTK_PP_4_SGXOSTimer_getregister();              \
	if (mtkpp_data != NULL) {                               \
	mtkpp_data->pfn_print(mtkpp_data, __VA_ARGS__); \
	} else {                                                \
	PVR_DPF((level, __VA_ARGS__));                  \
	}                                                       \
	} while (0)
#define _PVR_LOG_MTKPP(...)                                             \
	do {                                                            \
	MTK_PROC_PRINT_DATA *mtkpp_data =                       \
	MTK_PP_4_SGXOSTimer_getregister();              \
	if (mtkpp_data != NULL) {                               \
