...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 36)
static int bc_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
	    unsigned long arg);
#else
static long bc_ioctl(struct file *file, unsigned int cmd, unsigned long arg);
static long bc_ioctl_unlocked(struct file *file, unsigned int cmd,
	      unsigned long arg);
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 36))
...
static IMG_VOID BCFreeContigMemory(unsigned long ulSize, IMG_HANDLE hMemHandle,
...
#define OPEN_FXN(id)                                                  \
	static PVRSRV_ERROR OpenBCDevice##id(IMG_UINT32 ui32DeviceID, \
	     IMG_HANDLE *phDevice)    \
	{                                                             \
	BC_CAT_DEVINFO *psDevInfo;                            \
...
static PVRSRV_ERROR GetBCBuffer(IMG_HANDLE hDevice, IMG_UINT32 ui32BufferNumber,
	PVRSRV_SYNC_DATA *psSyncData,
	IMG_HANDLE *phBuffer)
{
	BC_CAT_DEVINFO *psDevInfo;
...
static PVRSRV_ERROR
GetBCBufferAddr(IMG_HANDLE hDevice, IMG_HANDLE hBuffer,
...
static int BC_CreateBuffers(int id, bc_buf_params_t *p)
{
	...
	if ((pixel_fmt == PVRSRV_PIXEL_FORMAT_I420) ||
	    (pixel_fmt == PVRSRV_PIXEL_FORMAT_YV12)) {
	...
	CpuPAddrToSysPAddrBC(paddr);
	psDevInfo->psSystemBuffer[i].sPageAlignSysAddr.uiAddr =
	psDevInfo->psSystemBuffer[i].sSysAddr.uiAddr &
	0xFFFFF000;
	}
#endif
...
*/
...
#if defined(BC_DISCONTIG_BUFFERS)
	BCFreeDiscontigMemory(
	psDevInfo->psSystemBuffer[i].ulSize,
	psDevInfo->psSystemBuffer[i].hMemHandle,
	psDevInfo->psSystemBuffer[i].sCPUVAddr,
	psDevInfo->psSystemBuffer[i].psSysAddr);
#else
	...
	psDevInfo->sBufferInfo.pixelformat =
	...
	if (psDevInfo == NULL) {
	psDevInfo = (BC_CAT_DEVINFO *)BCAllocKernelMem(
	sizeof(BC_CAT_DEVINFO));
	...
	if (BCOpenPVRServices(&psDevInfo->hPVRServices) !=
	    PVRSRV_OK)
	return PVRSRV_ERROR_INIT_FAILURE;

	if (BCGetLibFuncAddr(psDevInfo->hPVRServices,
	...
	psDevInfo->psSystemBuffer = BCAllocKernelMem(
	sizeof(BC_CAT_BUFFER) * BC_MAX_NUM_BUFFERS);
	...
	if (psJTable->pfnPVRSRVRemoveBCDevice(
	    psDevInfo->ulDeviceID) != PVRSRV_OK)
	//return PVRSRV_ERROR_GENERIC;
	return 1;

	if (BCClosePVRServices(psDevInfo->hPVRServices) !=
	    PVRSRV_OK) {
	...
	if (IS_ERR(bc_class)) {
	printk(KERN_ERR DRVNAME
	       ": upable to create device class\n");
	goto ExitUnregister;
	}

	for (id = 0; id < DEVICE_COUNT; id++) {
	bc_dev = device_create(bc_class, NULL, MKDEV(major, id),
	       NULL, DEVNAME "%d", id);

	if (IS_ERR(bc_dev)) {
	printk(KERN_ERR DRVNAME
	       ": unable to create device %d\n",
	       id);
...
ExitDisable:
	...
	printk(KERN_ERR DRVNAME
	       ": can't free texture buffers\n");
	return;
	}
	if (BC_Unregister(id) != PVRSRV_OK) {
	printk(KERN_ERR DRVNAME
	       ": can't un-register BC service\n");
	return;
	}
	}
	}
...
#define VMALLOC_TO_PAGE_PHYS(vAddr) page_to_phys(vmalloc_to_page(vAddr))
	...
	LinAddr = __vmalloc(ulSize, GFP_KERNEL | __GFP_HIGHMEM,
	...
	pPhysAddr =
	kmalloc(ulPages * sizeof(IMG_SYS_PHYADDR), GFP_KERNEL);
	if (!pPhysAddr) {
	vfree(LinAddr);
	return BCE_ERROR_OUT_OF_MEMORY;
	}
	...
	*pLinAddr = LinAddr;
	...
	pPhysAddr[ulPage].uiAddr =
	VMALLOC_TO_PAGE_PHYS(LinAddr);
	   ...
	   IMG_CPU_PHYADDR PhysAddr)
{
	...
	*phPVRServices = 0;
	...
	static PVRSRV_ERROR BCGetLibFuncAddr(
	IMG_HANDLE unref__ hExtDrv, IMG_CHAR * szFunctionName,
	PFN_BC_GET_PVRJTABLE * ppfnFuncTable)
	{
	...
	*ppfnFuncTable = PVRGetBufferClassJTable;
	...
	int ReconfigureBuffer(int id, bc_buf_params_t *p,
	      unsigned int *uiSucceed)
	{
	BCE_ERROR eError;
...
static long bc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
	...
	if (!access_ok(VERIFY_WRITE, params,
	       sizeof(BCIO_package)))
	return -EFAULT;
	...
	params->output =
	devinfo->psSystemBuffer[idx].sSysAddr.uiAddr;
	break;
	}
