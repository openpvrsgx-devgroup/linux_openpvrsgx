...
static void print_bvparams(struct bvbltparams *bltparams,
	   unsigned int pSrc1DescInfo,
	   unsigned int pSrc2DescInfo)
{
	...
	printk(KERN_INFO "%s: dst %d,%d rect{%d,%d sz %d,%d}"
	 " stride %ld desc 0x%p\n",
	       "bv", bltparams->dstgeom->width, bltparams->dstgeom->height,
	...
	printk(KERN_INFO "%s: src1 %d,%d rect{%d,%d sz %d,%d}"
	 " stride %ld, virtaddr 0x%x (0x%x)\n",
	       ...
	       bltparams->src1rect.left, bltparams->src1rect.top,
	       bltparams->src1rect.width, bltparams->src1rect.height,
	       bltparams->src1geom->virtstride,
	...
	printk(KERN_INFO "%s: src2 %d,%d rect{%d,%d sz %d,%d}"
	 " stride %ld, virtaddr 0x%x (0x%x)\n",
	       "bv", bltparams->src2geom->width, bltparams->src2geom->height,
	       bltparams->src2rect.left, bltparams->src2rect.top,
	       bltparams->src2rect.width, bltparams->src2rect.height,
	       bltparams->src2geom->virtstride,
...
static OMAPLFB_ERROR InitBltFBsCommon(OMAPLFB_DEVINFO *psDevInfo)
{
	...
	psPVRFBInfo->psBltFBsBvHndl =
	kzalloc(n * sizeof(*psPVRFBInfo->psBltFBsBvHndl), GFP_KERNEL);
	if (!psPVRFBInfo->psBltFBsBvHndl) {
	return OMAPLFB_ERROR_INIT_FAILURE;
	}

	psPVRFBInfo->psBltFBsBvPhys =
	kzalloc(n * sizeof(*psPVRFBInfo->psBltFBsBvPhys), GFP_KERNEL);
...
{
	OMAPLFB_FBINFO *psPVRFBInfo = &psDevInfo->sFBInfo;
	IMG_UINT uiBltFBSize = psDevInfo->sFBInfo.ulHeight *
	       psDevInfo->psLINFBInfo->fix.line_length;
	...
	psPVRFBInfo->uiBltFBsByteStride =
	psDevInfo->psLINFBInfo->fix.line_length;
	...
	pPaddrs = kzalloc(sizeof(*pPaddrs) * uiNumPages, GFP_KERNEL);
	if (!pPaddrs) {
	...
	pBvDesc->length =
	pBvPhysDesc->pagecount * pBvPhysDesc->pagesize;
...
static PVRSRV_ERROR InitBltFBsMapTiler2D(OMAPLFB_DEVINFO *psDevInfo)
{
	...
	pPageList =
	kzalloc(wpages * h * sizeof(*pPageList), GFP_KERNEL);
	if (!pPageList) {
...
static OMAPLFB_ERROR InitBltFBsTiler2D(OMAPLFB_DEVINFO *psDevInfo)
{
	...
	.fmt = psPVRFBInfo->uiBytesPerPixel == 2 ?
	...
	psPVRFBInfo->uiBltFBsByteStride = PAGE_ALIGN(
	psLINFBInfo->var.xres * psPVRFBInfo->uiBytesPerPixel);

	/* TILER will align width to 128-bytes */
	/* however, SGX must have full page width */
	w = ALIGN(psLINFBInfo->var.xres,
	  PAGE_SIZE / psPVRFBInfo->uiBytesPerPixel);
	...
	if (res < 0) {
	printk(KERN_ERR DRIVER_PREFIX "Could not allocate BltFBs\n");
	return OMAPLFB_ERROR_INIT_FAILURE;
	}
...
static struct bvbuffdesc *GetBvDescriptor(OMAPLFB_DEVINFO *psDevInfo,
	  ...
	  IMG_UINT32 ui32NumMemInfos)
{
	...
	psDevInfo->sPVRJTable.pfnPVRSRVDCMemInfoGetBvHandle(
	ppsMemInfos[ui32Idx], (IMG_VOID **)&pBvDesc);
	      ...
	      struct rgz_blt_entry *cur_entry, int is_last_blit)
{
	...
	(prev_bltparams->src1.desc == cur_bltparams->src1.desc ?
	 0 :
	 BVBATCH_SRC1) |
	(prev_bltparams->src2.desc == cur_bltparams->src2.desc ?
	 0 :
	 BVBATCH_SRC2) |
	(bvrect_same_origin(&prev_bltparams->src1rect,
	    &cur_bltparams->src1rect) ?
	 0 :
	 BVBATCH_SRC1RECT_ORIGIN) |
	(bvrect_same_origin(&prev_bltparams->src2rect,
	    &cur_bltparams->src2rect) ?
	 0 :
	 BVBATCH_SRC2RECT_ORIGIN) |
	(bvrect_same_origin(&prev_bltparams->dstrect,
	    &cur_bltparams->dstrect) ?
	 0 :
	 BVBATCH_DSTRECT_ORIGIN) |
	(bvrect_same_size(&prev_bltparams->src1rect,
	  &cur_bltparams->src1rect) ?
	 0 :
	 BVBATCH_SRC1RECT_SIZE) |
	(bvrect_same_size(&prev_bltparams->src2rect,
	  &cur_bltparams->src2rect) ?
	 0 :
	 BVBATCH_SRC2RECT_SIZE) |
	(bvrect_same_size(&prev_bltparams->dstrect,
	  &cur_bltparams->dstrect) ?
	 0 :
	    ...
	    IMG_UINT32 ui32NumMemInfos)
{
	...
	/* Destination data */
	entry->dstgeom.virtstride =
	psDevInfo->sFBInfo.uiBltFBsByteStride;
	entry->bp.dstdesc =
	psPVRFBInfo->psBltFBsBvHndl[psPVRFBInfo->iBltFBsIdx];
	...
	WARN(1,
	     "%s: Unable to determine scr2 buffer\n",
	     __func__);
	continue;
	}
	} else {
	...
	if (rgz_items > 1) {
	OMAPLFBDoBatching(prev_entry, entry,
	  j >= rgz_items - 1);
	prev_entry = entry;
	}
	...
	print_bvparams(&entry->bp, iSrc1DescInfo,
	       iSrc2DescInfo);
	}
...
{
	return (OMAPLFB_BLT_FBS_VRAM) ? InitBltFBsVram(psDevInfo) :
	InitBltFBsTiler2D(psDevInfo);
}
