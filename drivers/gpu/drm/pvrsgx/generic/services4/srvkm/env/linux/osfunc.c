...
OSAllocPages_Impl(IMG_UINT32 ui32AllocFlags, IMG_UINT32 ui32Size,
	  ...
	  IMG_UINT32 ui32PrivDataLength, IMG_VOID **ppvCpuVAddr,
	       ...
	       IMG_UINT32 *ui32NumAddrOffsets)
{
	...
	return GetIONLinuxMemAreaInfo(psLinuxMemArea, pui32AddressOffsets,
	      ui32NumAddrOffsets);
}
...
int OSGetProcCmdline(IMG_UINT32 ui32PID, char *buffer, int buff_size)
{
	...
	len = buff_size - res;
	res += pvr_access_process_vm(task, mm->env_start,
	     buffer + res, len, 0);
...
{
	PVR_TRACE(("%s: unmapping %d bytes from %p", __FUNCTION__, ui32Bytes,
	   pvLinAddr));
...
OSReservePhys(IMG_CPU_PHYADDR BasePAddr, IMG_UINT32 ui32Bytes,
	      IMG_UINT32 ui32MappingFlags, IMG_VOID **ppvCpuVAddr,
	      IMG_HANDLE *phOSMemHandle)
{
	LinuxMemArea *psLinuxMemArea;
...
static unsigned long VMallocAreaToPhys(LinuxMemArea *psLinuxMemArea,
	       ...
	       IMG_UINT32 ui32PageNum)
{
	return vmalloc_to_pfn(pvRangeAddrStart + ui32PageNum * PAGE_SIZE)
	       << PAGE_SHIFT;
}
	  ...
	  IMG_UINT32 ui32PageNum)
{
	...
	.pvPageList[ui32PageNumOffset + ui32PageNum];
	    ...
	    OuterCacheOp_t pfnOuterCacheOp)
{
	...
	ulStart = pfnMemAreaToPhys(psLinuxMemArea,
	   pvRangeAddrStart,
	   ui32PageNumOffset, i);
...
{
	if (ui32Length)
	dma_cache_wback_inv((IMG_UINTPTR_T)pvRangeAddrStart,
	    ui32Length);
	return IMG_TRUE;
}
