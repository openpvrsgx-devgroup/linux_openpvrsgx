...
PVRSRV_ERROR EnableSystemClocks(SYS_DATA *psSysData)
{
	...
	if (err) {
	PVR_DPF((PVR_DBG_ERROR, "%s failed to power up GPU: %d",
	 __func__, err));
	...
	if (err) {
	PVR_DPF((PVR_DBG_ERROR, "%s failed to ungate GPU clock: %d",
	 __func__, err));
	...
	if (err) {
	PVR_DPF((PVR_DBG_ERROR, "%s failed to initialise GPU clock: %d",
	 __func__, err));
	return PVRSRV_ERROR_CLOCK_REQUEST_FAILED;
	}
