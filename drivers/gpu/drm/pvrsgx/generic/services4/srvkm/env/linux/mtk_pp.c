	   ...
	   ...)
{
	...
	/* print string */
	va_start(args, fmt);
	len = vsnprintf(buf, (data->data_array_size - data->current_data), fmt,
	args);
	va_end(args);
...
static void MTKPP_AllocData(MTK_PROC_PRINT_DATA *data, int data_size,
	    int max_line)
{
	MTKPP_Lock(data);
...
static void *MTKPP_SeqStart(struct seq_file *s, loff_t *pos)
{
	...
	spin_lock_irqsave(&g_MTKPP_4_SGXDumpDebugInfo_lock,
	  g_MTKPP_4_SGXDumpDebugInfo_irqflags);
...
{
	spin_unlock_irqrestore(&g_MTKPP_4_SGXDumpDebugInfo_lock,
	       g_MTKPP_4_SGXDumpDebugInfo_irqflags);
...
static struct seq_operations g_MTKPP_seq_ops = { .start = MTKPP_SeqStart,
	 .next = MTKPP_SeqNext,
	 .stop = MTKPP_SeqStop,
	 .show = MTKPP_SeqShow };
...
void MTKPP_Init(void)
{
	...
	{ MTKPP_ID_SGXDumpDebugInfo, MTKPP_BUFFERTYPE_QUEUEBUFFER, 0,
	  0 },
	...
	g_MTKPPdata[i] =
	MTKPP_AllocStruct(mtk_pp_register_tabls[i].type);

	if (g_MTKPPdata[i] == NULL) {
	_MTKPP_DEBUG_LOG("%s: alloc struct fail: flags = %d",
	 __func__,
	 mtk_pp_register_tabls[i].type);
	goto err_out;
	}
...
void MTKPP_4_SGXDumpDebugInfo_Aquire(void)
{
	...
	PVR_DPF((PVR_DBG_ERROR,
	 "%s: fail to allocate 512 KB for gpulog", __func__));
	}
