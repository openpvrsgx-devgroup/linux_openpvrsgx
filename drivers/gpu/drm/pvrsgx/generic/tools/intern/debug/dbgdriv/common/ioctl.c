...
#else
	*ppvOut = ExtDBGDrivCreateStream(psIn->u.pszName, psIn->ui32CapMode,
	 psIn->ui32OutMode,
	       ...
	       IMG_VOID *pvOutBuffer)
{
	...
	*phStream = PStream2SID(ExtDBGDrivFindStream(psParams->u.pszName,
	 ...
	 IMG_VOID *pvOutBuffer)
{
	...
	*pui32OutLen = ExtDBGDrivWriteString(
	   ...
	   IMG_VOID *pvOutBuffer)
{
	...
	psStream = SID2PStream(psParams->hStream);
	if (psStream != (PDBG_STREAM)IMG_NULL) {
	*pui32OutLen = ExtDBGDrivWriteStringCM(
	psStream, psParams->u.pszString, psParams->ui32Level);
	return (IMG_TRUE);
	} else {
	*pui32OutLen = 0;
...
static IMG_UINT32 DBGDIOCDrivWrite(IMG_VOID *pvInBuffer, IMG_VOID *pvOutBuffer)
{
	...
	if (psStream != (PDBG_STREAM)IMG_NULL) {
	*pui32BytesCopied = ExtDBGDrivWrite(
	psStream, psInParams->u.pui8InBuffer,
...
static IMG_UINT32 DBGDIOCDrivWrite2(IMG_VOID *pvInBuffer, IMG_VOID *pvOutBuffer)
{
	...
	psStream = SID2PStream(psInParams->hStream);
	if (psStream != (PDBG_STREAM)IMG_NULL) {
	*pui32BytesCopied = ExtDBGDrivWrite2(
	psStream, psInParams->u.pui8InBuffer,
	psInParams->ui32TransferSize, psInParams->ui32Level);
	return (IMG_TRUE);
	} else {
	     ...
	     IMG_VOID *pvOutBuffer)
{
	...
	psStream = SID2PStream(psInParams->hStream);
	if (psStream != (PDBG_STREAM)IMG_NULL) {
	*pui32BytesCopied = ExtDBGDrivWriteCM(
	psStream, psInParams->u.pui8InBuffer,
	psInParams->ui32TransferSize, psInParams->ui32Level);
	return (IMG_TRUE);
	} else {
...
static IMG_UINT32 DBGDIOCDrivRead(IMG_VOID *pvInBuffer, IMG_VOID *pvOutBuffer)
{
	...
	if (psStream != (PDBG_STREAM)IMG_NULL) {
	*pui32BytesCopied =
	ExtDBGDrivRead(psStream, psInParams->bReadInitBuffer,
	    ...
	    IMG_VOID *pvOutBuffer)
{
	...
	psStream = SID2PStream(psParams->hStream);
	if (psStream != (PDBG_STREAM)IMG_NULL) {
	ExtDBGDrivSetCaptureMode(psStream, psParams->ui32Mode,
	 psParams->ui32Start, psParams->ui32End,
	     ...
	     IMG_VOID *pvOutBuffer)
{
	...
	psStream = SID2PStream(psInParams->hStream);
	if (psStream != (PDBG_STREAM)IMG_NULL) {
	*pui32BytesCopied = ExtDBGDrivWriteLF(
	psStream, psInParams->u.pui8InBuffer,
	psInParams->ui32BufferSize, psInParams->ui32Level,
...
static IMG_UINT32 DBGDIOCDrivReadLF(IMG_VOID *pvInBuffer, IMG_VOID *pvOutBuffer)
{
	...
	psStream = SID2PStream(psInParams->hStream);
	if (psStream != (PDBG_STREAM)IMG_NULL) {
	*pui32BytesCopied = ExtDBGDrivReadLF(
	psStream, psInParams->ui32OutBufferSize,
	psInParams->u.pui8OutBuffer);
	return (IMG_TRUE);
	} else {
	*pui32BytesCopied = 0;
