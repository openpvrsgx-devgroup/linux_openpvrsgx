...
PVRSRV_ERROR SysInitialise(IMG_VOID)
{
	...
	PVR_DPF((PVR_DBG_ERROR,
	 "G3D failed to find g3d clock source-enable"));
	return PVRSRV_ERROR_INIT_FAILURE;
	}
	...
	*/
	eError = PVRSRVRegisterDevice(gpsSysData, SGXRegisterDevice, 1,
	      &gui32SGXDeviceID);
	if (eError != PVRSRV_OK) {
	PVR_DPF((PVR_DBG_ERROR,
	...
	*/
	...
	case HEAP_ID(PVRSRV_DEVICE_TYPE_SGX,
	     SGX_GENERAL_HEAP_ID): {
	...
	case HEAP_ID(PVRSRV_DEVICE_TYPE_SGX,
	     SGX_KERNEL_CODE_HEAP_ID): {
	pStr = "KernelCodeHeapPageSize";
	break;
	}
	case HEAP_ID(PVRSRV_DEVICE_TYPE_SGX,
	     SGX_KERNEL_DATA_HEAP_ID): {
	...
	case HEAP_ID(PVRSRV_DEVICE_TYPE_SGX,
	     SGX_VERTEXSHADER_HEAP_ID): {
	pStr = "VertexShaderHeapPageSize";
	break;
	}
	case HEAP_ID(PVRSRV_DEVICE_TYPE_SGX,
	     SGX_PDSPIXEL_CODEDATA_HEAP_ID): {
	pStr = "PDSPixelHeapPageSize";
	break;
	}
	case HEAP_ID(PVRSRV_DEVICE_TYPE_SGX,
	     SGX_PDSVERTEX_CODEDATA_HEAP_ID): {
	pStr = "PDSVertexHeapPageSize";
	break;
	}
	case HEAP_ID(PVRSRV_DEVICE_TYPE_SGX,
	     SGX_SYNCINFO_HEAP_ID): {
	pStr = "SyncInfoHeapPageSize";
	break;
	}
	case HEAP_ID(PVRSRV_DEVICE_TYPE_SGX,
	     SGX_3DPARAMETERS_HEAP_ID): {
	...
	if (pStr &&
	    OSReadRegistryDWORDFromString(
	    0, PVRSRV_REGISTRY_ROOT, pStr,
	    &psDeviceMemoryHeap[i]
	     .ui32DataPageSize) ==
	    IMG_TRUE) {
	PVR_DPF((
	PVR_DBG_VERBOSE,
	"SysInitialise: set Heap %s page size to %d",
	pStr,
	psDeviceMemoryHeap[i]
	.ui32DataPageSize));
	}
...
#if defined(SYS_USING_INTERRUPTS)
	if (psSysSpecData->ui32SysSpecificData &
