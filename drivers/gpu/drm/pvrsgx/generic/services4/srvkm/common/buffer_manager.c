	...
	IMG_BOOL bFromAllocator)
{
	...
	pBuf->DevVAddr.uiAddr, IMG_FALSE);
	}
	} else {
	if ((pBuf->ui32ExportCount == 0) &&
	    (pBuf->ui32RefCount == 0)) {
	...
	IMG_DEV_VIRTADDR *pDevVAddr)
{
	...
	*pDevVAddr = pMapping->DevVAddr;
	...
	psDeviceNode->pfnMMUMapShadow(pMapping->pBMHeap->pMMUHeap,
	      ...
	      pMapping->uSize,
	      pMapping->CpuVAddr,
	      pMapping->hOSMemHandle, pDevVAddr,
	      uFlags, (IMG_HANDLE)pMapping);
	break;
	}
	case hm_wrapped_scatter:
	case hm_wrapped_scatter_virtaddr: {
...
static IMG_VOID XProcWorkaroundFreeShareable(IMG_HANDLE hOSMemHandle)
{
	...
	PVR_DPF((PVR_DBG_VERBOSE, "Reduced refcount of SI[%d] from %d to %d",
	 ui32SI, gXProcWorkaroundShareData[ui32SI].ui32RefCount + 1,
	 gXProcWorkaroundShareData[ui32SI].ui32RefCount));
	...
	if (gXProcWorkaroundShareData[ui32SI].pvCpuVAddr !=
	    IMG_NULL) {
	OSUnReservePhys(
	gXProcWorkaroundShareData[ui32SI]
	.pvCpuVAddr,
	gXProcWorkaroundShareData[ui32SI]
	.ui32Size,
	gXProcWorkaroundShareData[ui32SI]
...
#define MAX(a, b) ((a) > (b) ? (a) : (b))
	...
	if (OSReservePhys(pMapping->CpuPAddr, uPSize, ui32Attribs,
	  &pMapping->CpuVAddr,
	  &pMapping->hOSMemHandle) != PVRSRV_OK) {
	PVR_DPF((PVR_DBG_ERROR,
