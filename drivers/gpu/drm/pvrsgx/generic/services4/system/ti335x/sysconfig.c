...
static IMG_CHAR *SysCreateVersionString(void)
{
	...
	gsSGXDeviceMap.sRegsCpuPBase, SYS_TI335x_SGX_REGS_SIZE,
	PVRSRV_HAP_UNCACHED | PVRSRV_HAP_KERNEL_ONLY, IMG_NULL);
	if (!pvRegsLinAddr) {
	return IMG_NULL;
	}
...
#if !defined(NO_HARDWARE)
	OSUnMapPhysToLin(pvRegsLinAddr, SYS_TI335x_SGX_REGS_SIZE,
...
PVRSRV_ERROR SysDeinitialise(SYS_DATA *psSysData)
{
	...
	OSBaseFreeContigMemory(SYS_TI335x_SGX_REGS_SIZE,
	       gsSGXRegsCPUVAddr,
	       gsSGXDeviceMap.sRegsCpuPBase);
#else
#if defined(SGX_OCP_REGS_ENABLED)
