...
#define AWDEBUG(fmt, args...) \
	printk("[DC_SUNXI_DEBUG] [%d]%s, " fmt "\n", __LINE__, __func__, args);
#else
#define AWDEBUG(fmt, args...)
#endif

extern IMG_BOOL IMG_IMPORT
PVRGetDisplayClassJTable(PVRSRV_DC_DISP2SRV_KMJTABLE *psJTable);
...
static inline bool DontWaitForVSync(DC_SUNXI_DEVINFO *psDevInfo)
{
	...
	return atomic_read(&psDevInfo->sFlushCommands) ||
	       atomic_read(&psDevInfo->sLeaveVT);
#else
	return atomic_read(&psDevInfo->sFlushCommands);
#endif //SUPPORT_DRI_DRM
}
...
static void DC_SUNXIFlip(DC_SUNXI_DEVINFO *psDevInfo, DC_SUNXI_BUFFER *psBuffer)
{
	...
	printk(KERN_ERR DRIVER_PREFIX
	       ": %s: Device %u: fb_pan_display failed (Y Offset: %lu, Error: %d)\n",
	       __FUNCTION__, psDevInfo->uiFBDevID, psBuffer->ulYOffset,
	       res);
	}
...
{
	DC_SUNXI_BUFFER *psBuffer =
	container_of(psWork, DC_SUNXI_BUFFER, sWork);
...
static DC_SUNXI_ERROR DC_SUNXICreateSwapQueue(DC_SUNXI_SWAPCHAIN *psSwapChain)
{
	 ...
	 */
	psSwapChain->psWorkQueue =
	alloc_ordered_workqueue(DEVNAME, WQ_FREEZABLE | WQ_MEM_RECLAIM);
	if (psSwapChain->psWorkQueue == NULL) {
	printk(KERN_ERR DRIVER_PREFIX
	       ": %s: Device %u: Couldn't create workqueue\n",
...
static DC_SUNXI_DEVINFO *DC_SUNXIGetDevInfoPtr(unsigned uiFBDevID)
{
	...
	AWDEBUG("uiFBDevID = %d, but DC_SUNXI_MAX_NUM_DEVICES = %d ",
	uiFBDevID, DC_SUNXI_MAX_NUM_DEVICES);
	return NULL;
	}
...
static inline void DC_SUNXISetDevInfoPtr(unsigned uiFBDevID,
...
static DC_SUNXI_DEVINFO *DC_SUNXIGetDevInfoPtrByPVRDevID(unsigned uiPVRDevID)
{
	...
	if (gapsDevInfo[i] != NULL &&
	    gapsDevInfo[i]->uiPVRDevID == uiPVRDevID) {
...
int PVR_DRM_MAKENAME(DISPLAY_CONTROLLER, _Ioctl)(struct drm_device unref__ *dev,
	 void *arg,
	 struct drm_file unref__ *pFile)
{
...
#endif
	if (psDevInfo == NULL) {
	AWDEBUG("get DC_SUNXI_DEVINFO = NULL.  psArgs->dev = %d",
	psArgs->dev);
	return -EINVAL;
	}
	...
	if (bLeaveVT) {
	DC_SUNXIFlip(psDevInfo,
	     &psDevInfo->sSystemBuffer);
	}
	}
...
static inline bool SwapChainHasChanged(DC_SUNXI_DEVINFO *psDevInfo,
	       DC_SUNXI_SWAPCHAIN *psSwapChain)
{
...
extern int dispc_gralloc_queue_for_IMG(disp_layer_config *psDispcData,
	       int ui32DispcDataLength,
	       void (*cb_fn)(void *, int),
	       void *cb_arg);
static void QueueBufferImmediate(DC_SUNXI_DEVINFO *psDevInfo,
	 DC_SUNXI_BUFFER *psBuffer,
	 void (*cb_fn)(void *, int), void *cb_arg)
{
	...
	dispc_gralloc_queue_for_IMG(&sDispcData, sizeof(disp_layer_config),
	    cb_fn, cb_arg);
}
	      ...
	      IMG_UINT32 *pui32SwapChainID)
{
	...
	psSwapChain = (DC_SUNXI_SWAPCHAIN *)kmalloc(sizeof(DC_SUNXI_SWAPCHAIN),
	    GFP_KERNEL);
	if (!psSwapChain) {
	...
	psSwapChain->psBuffer = (DC_SUNXI_BUFFER *)kmalloc(
	sizeof(DC_SUNXI_BUFFER) * ui32BufferCount, GFP_KERNEL);
...
static IMG_BOOL ProcessFlipV2(IMG_HANDLE hCmdCookie,
	      ...
	      IMG_UINT32 ui32NumMemInfos, void *psDispcData,
	      IMG_UINT32 ui32DispcDataLength)
{
	...
	printk(KERN_WARNING DRIVER_PREFIX
	       ...
	       __FUNCTION__, psDevInfo->uiFBDevID);
	gapsDevInfo[0]->sPVRJTable.pfnPVRSRVCmdComplete(hCmdCookie,
	IMG_TRUE);
	return IMG_TRUE;
	}
	...
	dispc_gralloc_queue_for_IMG(psDispcData, ui32DispcDataLength,
	    dispc_proxy_cmdcomplete,
	    (void *)hCmdCookie);
...
static DC_SUNXI_DEVINFO *DC_SUNXIInitDev(unsigned uiFBDevID)
{
	...
	psDevInfo = (DC_SUNXI_DEVINFO *)kmalloc(sizeof(DC_SUNXI_DEVINFO),
	GFP_KERNEL);
	...
	if (psDevInfo->sPVRJTable.pfnPVRSRVRegisterCmdProcList(
	    ...
	    aui32SyncCountList, DC_SUNXI_COMMAND_COUNT) != PVRSRV_OK) {
	printk(KERN_ERR DRIVER_PREFIX
...
*/
	...
	AWDEBUG("DC_SUNXIInitDev return NULL. func:%s line:%d",
	__func__, __LINE__);
	return DC_SUNXI_ERROR_INIT_FAILURE;
	}
	return DC_SUNXI_OK;
