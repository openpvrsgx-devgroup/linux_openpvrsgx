	   ...
	   PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	psGetClientInfoOUT->eError = PVRSRVLookupHandle(
	psPerProc->psHandleBase, &hDevCookieInt,
	psGetClientInfoIN->hDevCookie, PVRSRV_HANDLE_TYPE_DEV_NODE);
	if (psGetClientInfoOUT->eError != PVRSRV_OK) {
	return 0;
	}

	psGetClientInfoOUT->eError = SGXGetClientInfoKM(
	hDevCookieInt, &psGetClientInfoOUT->sClientInfo);
	return 0;
}

static IMG_INT SGXReleaseClientInfoBW(
	...
	PVRSRV_BRIDGE_RETURN *psRetOUT, PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	psDevInfo = (PVRSRV_SGXDEV_INFO *)((PVRSRV_DEVICE_NODE *)hDevCookieInt)
	    ->pvDevice;
...
static IMG_INT SGXGetInternalDevInfoBW(
	IMG_UINT32 ui32BridgeID,
...
static IMG_INT SGXDoKickBW(IMG_UINT32 ui32BridgeID,
	   ...
	   PVRSRV_PER_PROCESS_DATA *psPerProc)
{
...
#else
	&psDoKickIN->sCCBKick.asTAStatusUpdate[i]
	 .hKernelMemInfo,
#endif
	psDoKickIN->sCCBKick.asTAStatusUpdate[i]
	.hKernelMemInfo,
...
#if defined(SUPPORT_SID_INTERFACE)
	sCCBKickKM.asTAStatusUpdate[i].sCtlStatus =
	psDoKickIN->sCCBKick.asTAStatusUpdate[i].sCtlStatus;
#endif
...
#else
	&psDoKickIN->sCCBKick.as3DStatusUpdate[i]
	 .hKernelMemInfo,
#endif
	psDoKickIN->sCCBKick.as3DStatusUpdate[i]
	.hKernelMemInfo,
...
#if defined(SUPPORT_SID_INTERFACE)
	sCCBKickKM.as3DStatusUpdate[i].sCtlStatus =
	psDoKickIN->sCCBKick.as3DStatusUpdate[i].sCtlStatus;
#endif
#else
	PVRSRVLookupHandle(
	psPerProc->psHandleBase,
	...
	PVR_DPF((PVR_DBG_ERROR,
	 "%s: SGXDoKickBW:"
	 " Invalid pasDstSyncHandles pointer",
	 __FUNCTION__));
	return -EFAULT;
	}
...
#if defined(SUPPORT_SID_INTERFACE)
	OSMemCopy(&sCCBKickKM.sCommand, &psDoKickIN->sCCBKick.sCommand,
	  sizeof(sCCBKickKM.sCommand));

	sCCBKickKM.ui32NumDstSyncObjects =
	psDoKickIN->sCCBKick.ui32NumDstSyncObjects;
	sCCBKickKM.ui32NumTAStatusVals =
	psDoKickIN->sCCBKick.ui32NumTAStatusVals;
	sCCBKickKM.ui32Num3DStatusVals =
	psDoKickIN->sCCBKick.ui32Num3DStatusVals;
...
static IMG_INT SGXScheduleProcessQueuesBW(
	IMG_UINT32 ui32BridgeID,
	    ...
	    PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	&psKick->hTASyncInfo,
#endif
	psKick->hTASyncInfo, PVRSRV_HANDLE_TYPE_SYNC_INFO);
	if (psRetOUT->eError != PVRSRV_OK) {
	return 0;
	}
	}
	...
	&psKick->h3DSyncInfo,
#endif
	psKick->h3DSyncInfo, PVRSRV_HANDLE_TYPE_SYNC_INFO);
	if (psRetOUT->eError != PVRSRV_OK) {
	return 0;
	}
	}
...
#endif
	psKick->ahSrcSyncInfo[i], PVRSRV_HANDLE_TYPE_SYNC_INFO);
	if (psRetOUT->eError != PVRSRV_OK) {
	return 0;
	}
	}
...
#endif
	psKick->ahDstSyncInfo[i], PVRSRV_HANDLE_TYPE_SYNC_INFO);
	if (psRetOUT->eError != PVRSRV_OK) {
	return 0;
	}
	}
...
static IMG_INT SGXSetTransferContextPriorityBW(
	IMG_UINT32 ui32BridgeID,
	PVRSRV_BRIDGE_IN_SGX_SET_TRANSFER_CONTEXT_PRIORITY
	*psSGXSetTransferContextPriorityIN,
	PVRSRV_BRIDGE_RETURN *psRetOUT, PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	IMG_HANDLE hDevCookieInt;
	...
	ui32BridgeID, PVRSRV_BRIDGE_SGX_SET_TRANSFER_CONTEXT_PRIORITY);
	...
	psRetOUT->eError = PVRSRVLookupHandle(
	psPerProc->psHandleBase, &hTransferContextInt,
	...
	hDevCookieInt, hTransferContextInt,
...
SGXSetRenderContextPriorityBW(IMG_UINT32 ui32BridgeID,
	      ...
	      *psSGXSetRenderContextPriorityIN,
	      PVRSRV_BRIDGE_RETURN *psRetOUT,
	      PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	IMG_HANDLE hDevCookieInt;
	...
	psRetOUT->eError = PVRSRVLookupHandle(
	psPerProc->psHandleBase, &hRenderContextInt,
	...
	hDevCookieInt, hRenderContextInt,
...
static IMG_INT SGXSubmit2DBW(IMG_UINT32 ui32BridgeID,
	     ...
	     PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	for (i = 0; i < psKick->ui32NumSrcSync; i++) {
	psRetOUT->eError = PVRSRVLookupHandle(
	psPerProc->psHandleBase, &psKick->ahSrcSyncInfo[i],
	psKick->ahSrcSyncInfo[i], PVRSRV_HANDLE_TYPE_SYNC_INFO);
	if (psRetOUT->eError != PVRSRV_OK) {
	return 0;
	}
	...
	&psKick->hDstSyncInfo,
#endif
	psKick->hDstSyncInfo, PVRSRV_HANDLE_TYPE_SYNC_INFO);
	if (psRetOUT->eError != PVRSRV_OK) {
	return 0;
	}
	}
	 ...
	 PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	psRetOUT->eError = PVRSRVLookupHandle(
	psPerProc->psHandleBase, &hDevMemContextInt,
	...
	psDevInfo, &sMiscInfo, psDeviceNode, hDevMemContextInt);
	  ...
	  PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	psSGXReadHWPerfCBOUT->eError = PVRSRVLookupHandle(
	psPerProc->psHandleBase, &hDevCookieInt,
	psSGXReadHWPerfCBIN->hDevCookie, PVRSRV_HANDLE_TYPE_DEV_NODE);
	...
	OSAllocMem(
	PVRSRV_OS_PAGEABLE_HEAP, ui32AllocatedSize,
	(IMG_VOID **)&psAllocated, &hAllocatedHandle,
	"Array of Hardware Performance Circular Buffer Data"));

	psSGXReadHWPerfCBOUT->eError = SGXReadHWPerfCBKM(
	hDevCookieInt, psSGXReadHWPerfCBIN->ui32ArraySize, psAllocated,
	&psSGXReadHWPerfCBOUT->ui32DataCount,
	&psSGXReadHWPerfCBOUT->ui32ClockSpeed,
	&psSGXReadHWPerfCBOUT->ui32HostTimeStamp);
	if (psSGXReadHWPerfCBOUT->eError == PVRSRV_OK) {
	psSGXReadHWPerfCBOUT->eError =
	CopyToUserWrapper(psPerProc, ui32BridgeID,
	  ...
	  PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	psSGXDevInitPart2OUT->eError = PVRSRVLookupHandle(
	psPerProc->psHandleBase, &hDevCookieInt,
	psSGXDevInitPart2IN->hDevCookie, PVRSRV_HANDLE_TYPE_DEV_NODE);
	...
	psPerProc->psHandleBase, &hDummy,
	psSGXDevInitPart2IN->sInitInfo
	.hKernelClearClipWAIndexStreamMemInfo,
	PVRSRV_HANDLE_TYPE_MEM_INFO);
	if (eError != PVRSRV_OK) {
	bLookupFailed = IMG_TRUE;
	}
	...
	eError = PVRSRVLookupHandle(psPerProc->psHandleBase, &hDummy,
	    psSGXDevInitPart2IN->sInitInfo
	    .hKernelVDMStateUpdateBufferMemInfo,
	    PVRSRV_HANDLE_TYPE_MEM_INFO);
	if (eError != PVRSRV_OK) {
	bLookupFailed = IMG_TRUE;
	}
	...
	psPerProc->psHandleBase,
	&psSGXDevInitPart2IN->sInitInfo
	 .hKernelVDMCtrlStreamBufferMemInfo,
	psSGXDevInitPart2IN->sInitInfo.hKernelVDMCtrlStreamBufferMemInfo,
	PVRSRV_HANDLE_TYPE_MEM_INFO);
	if (eError != PVRSRV_OK) {
	bReleaseFailed = IMG_TRUE;
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelCCBEventKickerMemInfo);
#else
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt,
	psSGXDevInitPart2IN->sInitInfo.hKernelCCBEventKickerMemInfo);
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelSGXHostCtlMemInfo);
#else
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt,
	psSGXDevInitPart2IN->sInitInfo.hKernelSGXHostCtlMemInfo);
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelSGXTA3DCtlMemInfo);
#else
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt,
	psSGXDevInitPart2IN->sInitInfo.hKernelSGXTA3DCtlMemInfo);
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelSGXPTLAWriteBackMemInfo);
#else
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt,
	psSGXDevInitPart2IN->sInitInfo.hKernelSGXPTLAWriteBackMemInfo);
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelHWProfilingMemInfo);
	if (eError != PVRSRV_OK) {
	bDissociateFailed = IMG_TRUE;
	}
#else
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelHWPerfCBMemInfo);
#else
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt,
	psSGXDevInitPart2IN->sInitInfo.hKernelHWPerfCBMemInfo);
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelTASigBufferMemInfo);
#else
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt,
	psSGXDevInitPart2IN->sInitInfo.hKernelTASigBufferMemInfo);
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernel3DSigBufferMemInfo);
#else
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt,
	psSGXDevInitPart2IN->sInitInfo.hKernel3DSigBufferMemInfo);
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelDummyTermStreamMemInfo);
	if (eError != PVRSRV_OK) {
	bDissociateFailed = IMG_TRUE;
	}
#else
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelClearClipWAPDSMemInfo);
	if (eError != PVRSRV_OK) {
	bDissociateFailed = IMG_TRUE;
	}
#else
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelClearClipWAUSEMemInfo);
	if (eError != PVRSRV_OK) {
	bDissociateFailed = IMG_TRUE;
	}
#else
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelClearClipWAParamMemInfo);
	if (eError != PVRSRV_OK) {
	bDissociateFailed = IMG_TRUE;
	}
#else
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelClearClipWAPMPTMemInfo);
	if (eError != PVRSRV_OK) {
	bDissociateFailed = IMG_TRUE;
	}
#else
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelClearClipWATPCMemInfo);
	if (eError != PVRSRV_OK) {
	bDissociateFailed = IMG_TRUE;
	}
#else
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelClearClipWAPSGRgnHdrMemInfo);
	if (eError != PVRSRV_OK) {
	bDissociateFailed = IMG_TRUE;
	}
#else
...
#if defined(SUPPORT_SID_INTERFACE)
	eError = PVRSRVDissociateDeviceMemKM(
	hDevCookieInt, asInitInfoKM.hKernelEDMStatusBufferMemInfo);
	if (eError != PVRSRV_OK) {
	bDissociateFailed = IMG_TRUE;
	}
#else
	...
	PVRSRVFreeDeviceMemKM(hDevCookieInt,
	      psSGXDevInitPart2IN->sInitInfo
	      .hKernelSGXPTLAWriteBackMemInfo);
#endif
	PVRSRVFreeDeviceMemKM(
	hDevCookieInt,
	psSGXDevInitPart2IN->sInitInfo.hKernelSGXMiscMemInfo);
	...
	if (hHandle == 0)
#else
	IMG_HANDLE hHandle = psSGXDevInitPart2IN->sInitInfo
	     .asInitMemHandles[i];
	...
	PVRSRVFreeDeviceMemKM(
	hDevCookieInt,
	(PVRSRV_KERNEL_MEM_INFO *)hHandle);
	}
	...
	asInitInfoKM.ui32ClientBuildOptions =
	psSGXDevInitPart2IN->sInitInfo.ui32ClientBuildOptions;
	asInitInfoKM.sSGXStructSizes =
	psSGXDevInitPart2IN->sInitInfo.sSGXStructSizes;
	asInitInfoKM.ui32CacheControl =
	psSGXDevInitPart2IN->sInitInfo.ui32CacheControl;
	asInitInfoKM.ui32EDMTaskReg0 =
	psSGXDevInitPart2IN->sInitInfo.ui32EDMTaskReg0;
	asInitInfoKM.ui32EDMTaskReg1 =
	psSGXDevInitPart2IN->sInitInfo.ui32EDMTaskReg1;
	asInitInfoKM.ui32ClkGateStatusReg =
	psSGXDevInitPart2IN->sInitInfo.ui32ClkGateStatusReg;
	asInitInfoKM.ui32ClkGateStatusMask =
	psSGXDevInitPart2IN->sInitInfo.ui32ClkGateStatusMask;
	...
	psSGXDevInitPart2OUT->eError =
	DevInitSGXPart2KM(psPerProc, hDevCookieInt, &asInitInfoKM);
#else
	psSGXDevInitPart2OUT->eError = DevInitSGXPart2KM(
	psPerProc, hDevCookieInt, &psSGXDevInitPart2IN->sInitInfo);
#endif
	     ...
	     PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	hHWRenderContextInt = SGXRegisterHWRenderContextKM(
	hDevCookieInt,
	       ...
	       PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	PVRSRV_BRIDGE_ASSERT_CMD(
	ui32BridgeID, PVRSRV_BRIDGE_SGX_UNREGISTER_HW_RENDER_CONTEXT);

	psRetOUT->eError = PVRSRVLookupHandle(
	psPerProc->psHandleBase, &hHWRenderContextInt,
...
static IMG_INT SGXRegisterHWTransferContextBW(
	IMG_UINT32 ui32BridgeID,
	...
	PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	PVRSRV_BRIDGE_ASSERT_CMD(
	ui32BridgeID, PVRSRV_BRIDGE_SGX_REGISTER_HW_TRANSFER_CONTEXT);
...
static IMG_INT SGXUnregisterHWTransferContextBW(
	IMG_UINT32 ui32BridgeID,
	...
	PVRSRV_BRIDGE_RETURN *psRetOUT, PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	PVRSRV_BRIDGE_ASSERT_CMD(
	ui32BridgeID, PVRSRV_BRIDGE_SGX_UNREGISTER_HW_TRANSFER_CONTEXT);

	psRetOUT->eError = PVRSRVLookupHandle(
	psPerProc->psHandleBase, &hHWTransferContextInt,
...
#if defined(SGX_FEATURE_2D_HARDWARE)
static IMG_INT SGXRegisterHW2DContextBW(
	IMG_UINT32 ui32BridgeID,
	...
	PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	psSGXRegHW2DContextOUT->eError = PVRSRVLookupHandle(
	psPerProc->psHandleBase, &hDevCookieInt,
	psSGXRegHW2DContextIN->hDevCookie, PVRSRV_HANDLE_TYPE_DEV_NODE);
	if (psSGXRegHW2DContextOUT->eError != PVRSRV_OK) {
	return 0;
	}

	hHW2DContextInt = SGXRegisterHW2DContextKM(
	hDevCookieInt, psSGXRegHW2DContextIN->pHW2DContextCpuVAddr,
	...
	&psSGXRegHW2DContextOUT->sHW2DContextDevVAddr, psPerProc);
...
static IMG_INT SGXUnregisterHW2DContextBW(
	IMG_UINT32 ui32BridgeID,
	...
	PVRSRV_BRIDGE_RETURN *psRetOUT, PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	psRetOUT->eError =
	PVRSRVLookupHandle(psPerProc->psHandleBase, &hHW2DContextInt,
	...
	hHW2DContextInt, psSGXUnregHW2DContextIN->bForceCleanup);
	if (psRetOUT->eError != PVRSRV_OK) {
	return 0;
	}

	psRetOUT->eError = PVRSRVReleaseHandle(
	psPerProc->psHandleBase, psSGXUnregHW2DContextIN->hHW2DContext,
	PVRSRV_HANDLE_TYPE_SGX_HW_2D_CONTEXT);
...
static IMG_INT SGX2DQueryBlitsCompleteBW(
	IMG_UINT32 ui32BridgeID,
	...
	PVRSRV_BRIDGE_RETURN *psRetOUT, PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	psRetOUT->eError = SGX2DQueryBlitsCompleteKM(
	psDevInfo, (PVRSRV_KERNEL_SYNC_INFO *)pvSyncInfo,
	ps2DQueryBltsCompleteIN->bWaitForComplete);

	return 0;
}

static IMG_INT SGXFindSharedPBDescBW(
	IMG_UINT32 ui32BridgeID,
	...
	PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	psSGXFindSharedPBDescOUT->eError, psPerProc,
	PVRSRV_BRIDGE_SGX_SHAREDPBDESC_MAX_SUBMEMINFOS + 4);
	...
	psSGXFindSharedPBDescOUT->eError = SGXFindSharedPBDescKM(
	psPerProc, hDevCookieInt,
	psSGXFindSharedPBDescIN->bLockOnFailure,
	psSGXFindSharedPBDescIN->ui32TotalPBSize, &hSharedPBDesc,
	&psSharedPBDescKernelMemInfo, &psHWPBDescKernelMemInfo,
	&psBlockKernelMemInfo, &psHWBlockKernelMemInfo,
	...
	psSharedPBDescKernelMemInfo, PVRSRV_HANDLE_TYPE_MEM_INFO_REF,
	...
	psHWPBDescKernelMemInfo, PVRSRV_HANDLE_TYPE_MEM_INFO_REF,
	PVRSRV_HANDLE_ALLOC_FLAG_MULTI,
	psSGXFindSharedPBDescOUT->hSharedPBDesc);
	...
	psBlockKernelMemInfo, PVRSRV_HANDLE_TYPE_MEM_INFO_REF,
	PVRSRV_HANDLE_ALLOC_FLAG_MULTI,
	psSGXFindSharedPBDescOUT->hSharedPBDesc);
...
static IMG_INT SGXUnrefSharedPBDescBW(
	IMG_UINT32 ui32BridgeID,
	...
	PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	PVRSRVLookupHandle(psPerProc->psHandleBase, &hSharedPBDesc,
...
static IMG_INT SGXAddSharedPBDescBW(
	IMG_UINT32 ui32BridgeID,
	...
	PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	eError = PVRSRVLookupHandle(psPerProc->psHandleBase, &hDevCookieInt,
	...
	eError = PVRSRVLookupHandle(
	psPerProc->psHandleBase, (IMG_VOID **)&psHWPBDescKernelMemInfo,
	...
	eError = PVRSRVLookupHandle(
	psPerProc->psHandleBase, (IMG_VOID **)&psHWBlockKernelMemInfo,
	...
	PVR_DPF((PVR_DBG_ERROR,
	 "%s: PVRSRV_BRIDGE_SGX_ADDSHAREDPBDESC:"
	 " Invalid phKernelMemInfos pointer",
	 __FUNCTION__));
	ret = -EFAULT;
	goto PVRSRV_BRIDGE_SGX_ADDSHAREDPBDESC_RETURN_RESULT;
	}
	...
	if (CopyFromUserWrapper(psPerProc, ui32BridgeID, phKernelMemInfoHandles,
	psSGXAddSharedPBDescIN->phKernelMemInfoHandles,
	...
	eError = SGXAddSharedPBDescKM(
	psPerProc, hDevCookieInt, psSharedPBDescKernelMemInfo,
	psHWPBDescKernelMemInfo, psBlockKernelMemInfo,
	psHWBlockKernelMemInfo, psSGXAddSharedPBDescIN->ui32TotalPBSize,
	&hSharedPBDesc, ppsKernelMemInfos,
	ui32KernelMemInfoHandlesCount,
...
static IMG_INT SGXGetInfoForSrvinitBW(
	IMG_UINT32 ui32BridgeID,
	...
	PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	psSGXInfoForSrvinitOUT->eError = PVRSRVLookupHandle(
	psPerProc->psHandleBase, &hDevCookieInt,
	psSGXInfoForSrvinitIN->hDevCookie, PVRSRV_HANDLE_TYPE_DEV_NODE);

	if (psSGXInfoForSrvinitOUT->eError != PVRSRV_OK) {
	return 0;
	}

	psSGXInfoForSrvinitOUT->eError = SGXGetInfoForSrvinitKM(
	hDevCookieInt,
#if defined(SUPPORT_SID_INTERFACE)
	&asHeapInfo[0], &psSGXInfoForSrvinitOUT->sInitInfo.sPDDevPAddr);
#else
	...
	IMG_BOOL bDumpPolls)
{
	...
	hUniqueTag = MAKEUNIQUETAG(
	(PVRSRV_KERNEL_MEM_INFO *)psBuffer->hKernelMemInfo);

#if defined(SUPPORT_SGX_NEW_STATUS_VALS)
	psCtrlMemInfoKM =
	((PVRSRV_KERNEL_MEM_INFO *)psBuffer->hCtrlKernelMemInfo);
	ui32Offset = psBuffer->sCtrlDevVAddr.uiAddr -
	     psCtrlMemInfoKM->sDevVAddr.uiAddr;
#else
	psCtrlMemInfoKM =
	((PVRSRV_KERNEL_MEM_INFO *)psBuffer->hKernelMemInfo)
	...
	PDUMPMEMUM(psPerProc, IMG_NULL, psBuffer->pvLinAddr,
	   (PVRSRV_KERNEL_MEM_INFO *)
	   psBuffer->hKernelMemInfo,
	   psBuffer->ui32Start,
	...
	if (bDumpPolls) {
	PDUMPMEMPOL(psCtrlMemInfoKM, ui32Offset, 0,
	    0xFFFFFFFF,
	    PDUMP_POLL_OPERATOR_NOTEQUAL, 0,
	    MAKEUNIQUETAG(psCtrlMemInfoKM));
	      ...
	      IMG_VOID *psBridgeOut, PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	if (OSAllocMem(PVRSRV_OS_PAGEABLE_HEAP, ui32BufferArraySize,
	       ...
	       "Array of Kick Tile Accelerator Dump Buffer") !=
	    PVRSRV_OK)
#else
	if (OSAllocMem(PVRSRV_OS_PAGEABLE_HEAP, ui32BufferArraySize,
	...
	eError = PVRSRVLookupHandle(psPerProc->psHandleBase, &pvMemInfo,
#if defined(SUPPORT_SID_INTERFACE)
	       ...
	       PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	PVRSRV_BRIDGE_ASSERT_CMD(
	ui32BridgeID, PVRSRV_BRIDGE_SGX_PDUMP_3D_SIGNATURE_REGISTERS);
	...
	PVR_DPF((
	PVR_DBG_ERROR,
	"PDumpTASignatureRegistersBW: hDevCookie lookup failed"));
	goto Exit;
	}
...
#if defined(PDUMP)
	PDUMPREGWITHFLAGS(SGX_PDUMPREG_NAME, EUR_CR_MASTER_CORE,
	  (SGX_FEATURE_MP_CORE_COUNT - 1)
	  << EUR_CR_MASTER_CORE_ENABLE_SHIFT,
	  psPDump3DSignatureRegistersIN->bLastFrame ?
	  PDUMP_FLAGS_LASTFRAME :
	  0);
#endif
#endif
	...
	PDump3DSignatureRegisters(
	&psDeviceNode->sDevId,
	psPDump3DSignatureRegistersIN->ui32DumpFrameNum,
	psPDump3DSignatureRegistersIN->bLastFrame, pui32Registers,
	psPDump3DSignatureRegistersIN->ui32NumRegisters);
...
static IMG_INT SGXPDumpCounterRegistersBW(
	IMG_UINT32 ui32BridgeID,
	...
	IMG_VOID *psBridgeOut, PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	PVR_DPF((
	PVR_DBG_ERROR,
	"SGXPDumpCounterRegistersBW: hDevCookie lookup failed"));
	ret = -ENOMEM;
	goto Exit;
	}
	       ...
	       PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	PVRSRV_BRIDGE_ASSERT_CMD(
	ui32BridgeID, PVRSRV_BRIDGE_SGX_PDUMP_TA_SIGNATURE_REGISTERS);
	   ...
	   PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	ui32MMUContextID, psPDumpHWPerfCBIN->ui32PDumpFlags);
	 ...
	 PVRSRV_PER_PROCESS_DATA *psPerProc)
{
	...
	PDumpSaveMemKM(&psDeviceNode->sDevId, &psPDumpSaveMem->szFileName[0],
	       ...
	       psPDumpSaveMem->sDevVAddr, psPDumpSaveMem->ui32Size,
	       ui32MMUContextID, psPDumpSaveMem->ui32PDumpFlags);
	return 0;
}
