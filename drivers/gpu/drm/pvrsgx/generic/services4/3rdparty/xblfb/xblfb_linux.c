...
int PVR_DRM_MAKENAME(DISPLAY_CONTROLLER,
	     _Suspend)(struct drm_device unref__ *dev)
{
	return 0;
}

int PVR_DRM_MAKENAME(DISPLAY_CONTROLLER,
	     _Resume)(struct drm_device unref__ *dev)
{
	return 0;
}
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 34))
#define XB_DISPLAY_DRIVER(drv, dev) \
	struct xb_display_driver *drv = (dev) != NULL ? (dev)->driver : NULL
#define XB_DISPLAY_MANAGER(man, dev) \
	struct xb_overlay_manager *man = (dev) != NULL ? (dev)->manager : NULL
...
XBLFB_ERROR XBLFBGetLibFuncAddr(char *szFunctionName,
...
void XBLFBQueueBufferForSwap(XBLFB_SWAPCHAIN *psSwapChain,
...
/* Insert a swap buffer into the swap chain work queue */
void XBLFBQueueBufferForSwap2(XBLFB_SWAPCHAIN *psSwapChain,
	      ...
	      XBLFB_HANDLE hCmdCookie)
{
	...
	printk(KERN_WARNING DRIVER_PREFIX
	       ": %s: Device %u: Cannot find the XBLFB_BUFFER for the virtual address.\n",
	       __FUNCTION__, psSwapChain->uiFBDevID);
	psSwapChain->psBuffer->psDevInfo->sPVRJTable
	.pfnPVRSRVCmdComplete((IMG_HANDLE)hCmdCookie, IMG_TRUE);
	return;
	}
	  ...
	  unsigned long event, void *data)
{
	...
	bBlanked = (*(IMG_INT *)psFBEvent->data != 0) ? XBLFB_TRUE :
	XBLFB_FALSE;
...
static void XBLFBEarlySuspendHandler(struct early_suspend *h)
{
	...
	if (psDevInfo != NULL) {
	XBLFBAtomicBoolSet(&psDevInfo->sEarlySuspendFlag,
	   XBLFB_TRUE);
