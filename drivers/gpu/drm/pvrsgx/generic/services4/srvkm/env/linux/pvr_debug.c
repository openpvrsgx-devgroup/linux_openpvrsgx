	 ...
	 const IMG_CHAR *pszFormat, va_list VArgs)
{
	...
	return (i32Len < 0 || i32Len >= (IMG_INT32)ui32Space) ? IMG_TRUE :
	IMG_FALSE;
}
	   ...
	   const IMG_CHAR *pszFormat, ...)
{
	...
	bTrace = (IMG_BOOL)(ui32DebugLevel & DBGPRIV_CALLTRACE) ? IMG_TRUE :
	  IMG_FALSE;
	...
	case DBGPRIV_FATAL: {
	strncpy(pszBuf,
	"PVR_K:(Fatal): ", (ui32BufSiz - 1));
	break;
	}
	case DBGPRIV_ERROR: {
	strncpy(pszBuf,
	"PVR_K:(Error): ", (ui32BufSiz - 1));
	break;
	}
	case DBGPRIV_WARNING: {
	strncpy(pszBuf,
	"PVR_K:(Warning): ", (ui32BufSiz - 1));
	break;
	}
	case DBGPRIV_MESSAGE: {
	strncpy(pszBuf,
	"PVR_K:(Message): ", (ui32BufSiz - 1));
	break;
	}
	case DBGPRIV_VERBOSE: {
	strncpy(pszBuf,
	"PVR_K:(Verbose): ", (ui32BufSiz - 1));
	break;
	}
	default: {
	strncpy(pszBuf, "PVR_K:(Unknown message level)",
	(ui32BufSiz - 1));
	break;
	}
	}
	...
	if (strlen(pszFullFileName) >
	    strlen(DEBUG_LOG_PATH_TRUNCATE) + 1)
	pszFileName =
	pszFullFileName +
	strlen(DEBUG_LOG_PATH_TRUNCATE) +
	1;
