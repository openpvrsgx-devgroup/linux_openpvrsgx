...
static inline void MTKLFBSetDevInfoPtr(unsigned uiFBDevID,
...
static inline MTKLFB_BOOL SwapChainHasChanged(MTKLFB_DEVINFO *psDevInfo,
...
static inline MTKLFB_BOOL DontWaitForVSync(MTKLFB_DEVINFO *psDevInfo)
{
	...
	bDontWait = bDontWait ||
	    MTKLFBAtomicBoolRead(&psDevInfo->sEarlySuspendFlag);
#endif
	      ...
	      IMG_UINT32 *pui32SwapChainID)
{
	...
	xlog_printk(
	ANDROID_LOG_WARN, DRIVER_PREFIX,
	DRIVER_PREFIX
	": %s: Device %u: Couldn't set frame buffer update mode %d\n",
	__FUNCTION__, psDevInfo->uiFBDevID,
	PVR_MTKFB3_UPDATE_MODE);
	}
#endif

	psSwapChain = (MTKLFB_SWAPCHAIN *)MTKLFBAllocKernelMem(
	sizeof(MTKLFB_SWAPCHAIN));
	if (!psSwapChain) {
	eError = PVRSRV_ERROR_OUT_OF_MEMORY;
	goto ExitUnLock;
	}

	psBuffer = (MTKLFB_BUFFER *)MTKLFBAllocKernelMem(sizeof(MTKLFB_BUFFER) *
	 ui32BufferCount);
	if (!psBuffer) {
	eError = PVRSRV_ERROR_OUT_OF_MEMORY;
	...
	xlog_printk(ANDROID_LOG_WARN, DRIVER_PREFIX,
	    DRIVER_PREFIX
...
void MTKLFBSwapHandler(MTKLFB_BUFFER *psBuffer)
{
	...
	int iBlankEvents =
	MTKLFBAtomicIntRead(&psDevInfo->sBlankEvents);
...
static MTKLFB_ERROR MTKLFBInitFBDev(MTKLFB_DEVINFO *psDevInfo)
{
	...
	if (res != 0) {
	xlog_printk(
	ANDROID_LOG_INFO, DRIVER_PREFIX,
	DRIVER_PREFIX
	": %s: Device %u: fb_set_var failed (Error: %d)\n",
	...
	xlog_printk(ANDROID_LOG_INFO, DRIVER_PREFIX,
	    DRIVER_PREFIX
...
static MTKLFB_DEVINFO *MTKLFBInitDev(unsigned uiFBDevID)
{
	...
	psDevInfo =
	(MTKLFB_DEVINFO *)MTKLFBAllocKernelMem(sizeof(MTKLFB_DEVINFO));

	if (psDevInfo == NULL) {
	xlog_printk(
	ANDROID_LOG_ERROR, DRIVER_PREFIX,
	DRIVER_PREFIX
	...
	if (psDevInfo->sPVRJTable.pfnPVRSRVRegisterCmdProcList(
	    ...
	    aui32SyncCountList, MTKLFB_COMMAND_COUNT) != PVRSRV_OK) {
	xlog_printk(
	ANDROID_LOG_ERROR, DRIVER_PREFIX,
	DRIVER_PREFIX
...
MTKLFB_ERROR MTKLFBInit(void)
{
	...
	if (MTKLFBGetLibFuncAddr("PVRGetDisplayClassJTable",
	 &gpfnGetPVRJTable) != MTKLFB_OK) {
...
static MTKLFB_BOOL MTKLFBDeInitDev(MTKLFB_DEVINFO *psDevInfo)
{
	...
	if (psPVRJTable->pfnPVRSRVRemoveCmdProcList(
	    psDevInfo->uiPVRDevID, MTKLFB_COMMAND_COUNT) != PVRSRV_OK) {
	xlog_printk(
	ANDROID_LOG_ERROR, DRIVER_PREFIX,
	DRIVER_PREFIX
