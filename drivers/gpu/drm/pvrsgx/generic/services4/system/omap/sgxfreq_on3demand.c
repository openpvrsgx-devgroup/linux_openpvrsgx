...
static struct sgxfreq_governor on3demand_gov = { .name = "on3demand",
...
static ssize_t show_load(struct device *dev, struct device_attribute *attr,
	 char *buf)
{
	return sprintf(buf, "%u\n", odd.load);
}

static DEVICE_ATTR(down_threshold, 0644, show_down_threshold,
...
static struct attribute *on3demand_attributes[] = {
	&dev_attr_down_threshold.attr, &dev_attr_up_threshold.attr,
	&dev_attr_history_size.attr, &dev_attr_load.attr, NULL
};
