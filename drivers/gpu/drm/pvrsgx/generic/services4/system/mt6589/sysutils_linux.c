...
IMG_VOID SysGetSGXTimingInformation(SGX_TIMING_INFORMATION *psTimingInfo)
{
	...
	mt_gpufreq_cur_freq() * 1000; // SYS_SGX_CLOCK_SPEED;
...
PVRSRV_ERROR EnableSGXClocks(SYS_DATA *psSysData)
{
	...
	{
	IMG_UINT32 val;
	DRV_WriteReg32(0xF0001200,
	       DRV_Reg32(0xF0001200) &
	       ~0x400); // disable MFG way_en
	val = DRV_Reg32(SPM_MFG_PWR_CON);
	val = (val & ~0x1) | 0x10;
	DRV_WriteReg32(SPM_MFG_PWR_CON,
	       val); // disable MFG clock and assert MFG reset
	DRV_WriteReg32(SPM_MFG_PWR_CON,
	       DRV_Reg32(SPM_MFG_PWR_CON) |
	       0x00000002); // enable MFG ISO
	OSWaitus(1);
	DRV_WriteReg32(SPM_MFG_PWR_CON,
	       DRV_Reg32(SPM_MFG_PWR_CON) &
	       0xFFFFFFFD); // disable MFG ISO
	DRV_WriteReg32(SPM_MFG_PWR_CON,
	       DRV_Reg32(SPM_MFG_PWR_CON) &
	       0xFFFFFFEF); // enable MFG clock
	DRV_WriteReg32(SPM_MFG_PWR_CON,
	       DRV_Reg32(SPM_MFG_PWR_CON) |
	       0x00000001); // dis-assert MFG reset
	OSWaitus(1);
	...
	DRV_WriteReg32(0xF0206008, 0xf); // MFG clock on
	OSWaitus(1);
	DRV_WriteReg32(0xF0001200, (DRV_Reg32(0xF0001200) & ~0x400) |
	   0x400); // enable MFG way_en
	}
