...
static int MRSTLFB_dimension_match(MRSTLFB_DEVINFO *psDevInfo,
...
static void MRSTLFBRestoreLastFlip(MRSTLFB_DEVINFO *psDevInfo)
{
	...
	MRSTLFBFlipToSurface(psDevInfo,
	     ...
	     MRSTLFB_BUFFER *psBuffer)
{
	...
	FlushInternalVSyncQueue(
	psDevInfo->psCurrentSwapChain,
	MRST_FALSE, dim_match);
	}
	...
	MRST_BOOL bFlushState)
{
	...
	if (psDevInfo->psCurrentSwapChain != NULL) {
	int dim_match = MRSTLFB_dimension_match(
	psDevInfo,
	psDevInfo->psCurrentSwapChain);
	FlushInternalVSyncQueue(
	psDevInfo->psCurrentSwapChain,
...
static IMG_VOID SetFlushState(MRSTLFB_DEVINFO *psDevInfo, MRST_BOOL bFlushState)
{
...
static MRST_ERROR UnblankDisplay(MRSTLFB_DEVINFO *psDevInfo)
{
	...
	console_unlock();
	if (res != 0) {
	printk(KERN_WARNING DRIVER_PREFIX ": fb_blank failed (%d)",
	       res);
...
GetDCBufferAddr(IMG_HANDLE hDevice, IMG_HANDLE hBuffer,
	IMG_SYS_PHYADDR **ppsSysAddr, IMG_SIZE_T *pui32ByteSize,
	      ...
	      IMG_UINT32 *pui32SwapChainID)
{
	...
	psSwapChain = (MRSTLFB_SWAPCHAIN *)MRSTLFBAllocKernelMem(
	sizeof(MRSTLFB_SWAPCHAIN));
	if (!psSwapChain) {
	...
	ppsBuffer = (MRSTLFB_BUFFER **)MRSTLFBAllocKernelMem(
	...
	psVSyncFlips = (MRSTLFB_VSYNC_FLIP_ITEM *)MRSTLFBAllocKernelMem(
	sizeof(MRSTLFB_VSYNC_FLIP_ITEM) * ulSwapChainLength);
	if (!psVSyncFlips) {
	eError = PVRSRV_ERROR_OUT_OF_MEMORY;
	...
	unsigned long bufSize = psDevInfo->sDisplayDim.ui32ByteStride *
	psDevInfo->sDisplayDim.ui32Height;
	if (MRSTLFBAllocBufferPages(psDevInfo->psDrmDevice, bufSize,
	    &ppsBuffer[i]) != MRST_OK) {
	eError = PVRSRV_ERROR_OUT_OF_MEMORY;
	       ...
	       IMG_HANDLE hSwapChain)
{
	...
	MRSTLFBFreeBufferPages(psDevInfo->psDrmDevice,
	       &psSwapChain->ppsBuffer[i]);
	}
...
static int MRSTLFBVSyncISR(struct drm_device *psDrmDevice, int iPipe)
{
...
int MRSTLFBHandleChangeFB(struct drm_device *dev, struct psb_framebuffer *psbfb)
{
	...
	psDevInfo->sDisplayFormat.pixelformat =
	(psbfb->base.depth == 16) ? PVRSRV_PIXEL_FORMAT_RGB565 :
	...
	if (psbfb->pvrBO != NULL) {
	psLinuxMemArea =
	(LinuxMemArea *)psbfb->pvrBO->sMemBlk.hOSMemHandle;
	}
	...
	psDevInfo->sSystemBuffer.uSysAddr.psNonCont =
	MRSTLFBAllocKernelMem(sizeof(IMG_SYS_PHYADDR) *
	      (psbfb->size >> PAGE_SHIFT));
	...
	psDevInfo->sSystemBuffer.uSysAddr.psNonCont[i]
	.uiAddr =
	...
	for (i = 0; i < psbfb->size >> PAGE_SHIFT; i++)
	psDevInfo->sSystemBuffer.uSysAddr.psNonCont[i]
	.uiAddr = page_to_pfn(page_list[i])
	  << PAGE_SHIFT;
	}
	}
...
static MRST_ERROR InitDev(MRSTLFB_DEVINFO *psDevInfo)
{
	...
	struct drm_psb_private *psDrmPrivate =
...
MRST_ERROR MRSTLFBInit(struct drm_device *dev)
{
	...
	psDevInfo = (MRSTLFB_DEVINFO *)MRSTLFBAllocKernelMem(
	sizeof(MRSTLFB_DEVINFO));
	...
	if (MRSTLFBGetLibFuncAddr("PVRGetDisplayClassJTable",
	  &pfnGetPVRJTable) != MRST_OK) {
	return (MRST_ERROR_INIT_FAILURE);
	}
	...
	if (MRSTLFBInstallVSyncISR(psDevInfo, MRSTLFBVSyncISR) !=
	    MRST_OK) {
	DEBUG_PRINTK((KERN_INFO DRIVER_PREFIX
	      "ISR Installation failed\n"));
	return (MRST_ERROR_INIT_FAILURE);
	}
#endif
...
MRST_ERROR MRSTLFBDeinit(void)
{
	...
	if (psDevInfo->sPVRJTable.pfnPVRSRVRemoveCmdProcList(
	    psDevInfo->uiDeviceID, MRSTLFB_COMMAND_COUNT) !=
	    PVRSRV_OK) {
	return (MRST_ERROR_GENERIC);
	}

	if (psDevInfo->sPVRJTable.pfnPVRSRVRegisterPowerDevice(
	    (IMG_UINT32)psDevInfo->uiDeviceID, IMG_NULL,
...
int MRSTLFBAllocBuffer(struct drm_device *dev, IMG_UINT32 ui32Size,
...

{
	...
	pvBuf = __vmalloc(
	ui32Size, GFP_KERNEL | __GFP_HIGHMEM | __GFP_ZERO,
	...
	*ppBuffer = NULL;
	...
	ret = psb_gtt_map_pvr_memory(
	dev, (unsigned int)*ppBuffer, (*ppBuffer)->ui32OwnerTaskID,
...
int MRSTLFBAllocBufferPages(struct drm_device *dev, IMG_UINT32 ui32Size,
	    MRSTLFB_BUFFER **ppBuffer)
{
	IMG_UINT32 ulPagesNumber;
	int i, ret;
	...
	*ppBuffer =
	MRSTLFBAllocKernelMem(sizeof(MRSTLFB_BUFFER) +
	      ulPagesNumber * sizeof(struct pages *));
	if (*ppBuffer == NULL) {
	return MRST_ERROR_OUT_OF_MEMORY;
	}
	...
	*ppBuffer = NULL;
	...
	for (i = 0; i < ulPagesNumber; i++) {
	(*ppBuffer)->uSysAddr.psNonCont[i].uiAddr =
	page_to_pfn((*ppBuffer)->buffer_pages[i]) << PAGE_SHIFT;
	}
...
int MRSTLFBFreeBufferPages(struct drm_device *dev, MRSTLFB_BUFFER **ppBuffer)
{
	...
	ulPagesNumber =
	((*ppBuffer)->ui32BufferSize + PAGE_SIZE - 1) / PAGE_SIZE;
	psb_gtt_unmap_pvr_memory(dev, (unsigned int)*ppBuffer,
	 (*ppBuffer)->ui32OwnerTaskID);
