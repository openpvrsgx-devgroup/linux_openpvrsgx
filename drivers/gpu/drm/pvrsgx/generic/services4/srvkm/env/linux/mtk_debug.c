...
static int MTKPPPrintTime(char *buf, int n)
{
	...
	return snprintf(buf, n, "[%5lu.%06lu] ", (unsigned long)t,
	nanosec_rem / 1000);
}
	 ...
	 ...)
{
	...
	buf += len;
	s = data->l;
	while (s < data->line_array_size && data->line[s] != NULL &&
	       data->line[s] <= buf) {
	data->line[s++] = NULL;
	}
...
static MTK_PROC_PRINT_DATA *MTKPPAllocStruct(int flags)
{
	...
	default:
	// something wrong
	PVR_DPF((PVR_DBG_ERROR, "%s: unknow flags: %d", __func__,
	 flags));
	goto err_out2;
	break;
	}
...
static void MTKPPAllocData(MTK_PROC_PRINT_DATA *data, int data_size,
	   int max_line)
{
	MTKPPLock(data);
...
static void *MTKPPSeqStart(struct seq_file *s, loff_t *pos)
{
	...
	spin_lock_irqsave(&g_MTKPP_4_SGXOSTimer_lock,
	  g_MTKPP_4_SGXOSTimer_irqflags);
...
{
	spin_unlock_irqrestore(&g_MTKPP_4_SGXOSTimer_lock,
	       g_MTKPP_4_SGXOSTimer_irqflags);
...
static struct seq_operations g_MTKPP_seq_ops = { .start = MTKPPSeqStart,
	 .next = MTKPPSeqNext,
	 .stop = MTKPPSeqStop,
	 .show = MTKPPSeqShow };
...
static IMG_VOID MTKPPInit(IMG_VOID)
{
	...
	PVR_DPF((PVR_DBG_ERROR,
	 "%s: index(%d) != tabel_uid(%d)", __func__, i,
	 mtk_pp_register_tabls[i].uid));
	goto err_out;
	}

	g_MTKPPdata[i] =
	MTKPPAllocStruct(mtk_pp_register_tabls[i].flags);

	if (g_MTKPPdata[i] == NULL) {
	PVR_DPF((PVR_DBG_ERROR,
	 "%s: alloc struct fail: flags = %d", __func__,
	 mtk_pp_register_tabls[i].flags));
	goto err_out;
	}

	if (mtk_pp_register_tabls[i].data_size > 0) {
	MTKPPAllocData(g_MTKPPdata[i],
...
{
	return (id >= 0 && id < MTK_PP_R_SIZE) ? g_MTKPPdata[id] : NULL;
}
...
int MTK_PP_4_SGXOSTimer_register(void)
{
	...
	MTKPPAllocData(g_MTKPPdata[MTK_PP_R_SGXOSTimer],
	       1024 * 1024 * 4, 1024 * 64);
	MTKPPCleanData(g_MTKPPdata[MTK_PP_R_SGXOSTimer]);
	}

	MTKPPPrintQueueBuffer2(g_MTKPPdata[MTK_PP_R_SGXOSTimer],
	       "=== start ===");
...
IMG_VOID MTKDebugSetInfo(const IMG_CHAR *pszInfo, IMG_INT32 i32Size)
{
...
IMG_VOID MTKDebugGetInfo(IMG_CHAR *acDebugMsg, IMG_INT32 i32Size)
{
	if (i32Size > MTK_DEBUG_MSG_LENGTH) {
	i32Size = MTK_DEBUG_MSG_LENGTH;
	}
	LinuxLockMutex(&g_sDebugMutex);
	if ((g_PID == OSGetCurrentProcessIDKM()) ||
	    (g_acMsgBuffer[0] == '\0')) {
