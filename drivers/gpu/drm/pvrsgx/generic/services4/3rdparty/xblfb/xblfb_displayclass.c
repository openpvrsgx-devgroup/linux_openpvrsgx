...
static inline void XBLFBSetDevInfoPtr(unsigned uiFBDevID,
...
static inline XBLFB_BOOL SwapChainHasChanged(XBLFB_DEVINFO *psDevInfo,
...
static inline XBLFB_BOOL DontWaitForVSync(XBLFB_DEVINFO *psDevInfo)
{
	...
	bDontWait = bDontWait ||
	    XBLFBAtomicBoolRead(&psDevInfo->sEarlySuspendFlag);
#endif
	      ...
	      IMG_UINT32 *pui32SwapChainID)
{
	...
	*/
	...
	psSwapChain =
	(XBLFB_SWAPCHAIN *)XBLFBAllocKernelMem(sizeof(XBLFB_SWAPCHAIN));
	if (!psSwapChain) {
	eError = PVRSRV_ERROR_OUT_OF_MEMORY;
	goto ExitUnLock;
	}

	psBuffer = (XBLFB_BUFFER *)XBLFBAllocKernelMem(sizeof(XBLFB_BUFFER) *
	       ui32BufferCount);
	if (!psBuffer) {
	eError = PVRSRV_ERROR_OUT_OF_MEMORY;
 ...
 */
static IMG_BOOL ProcessFlipV1(IMG_HANDLE hCmdCookie, XBLFB_DEVINFO *psDevInfo,
	      XBLFB_SWAPCHAIN *psSwapChain,
	      XBLFB_BUFFER *psBuffer,
 ...
 *
 */
static IMG_BOOL ProcessFlipV2(IMG_HANDLE hCmdCookie, XBLFB_DEVINFO *psDevInfo,
	      ...
	      IMG_UINT32 ui32DispcDataLength)
{
	...
	gapsDevInfo[0]->sPVRJTable.pfnPVRSRVDCMemInfoGetCpuVAddr(
	ppsMemInfos[0], &pViAddress);

	XBLFBQueueBufferForSwap2(psDevInfo->psSwapChain, pViAddress,
	 (XBLFB_HANDLE)hCmdCookie);
	}
...
static XBLFB_ERROR XBLFBInitFBDev(XBLFB_DEVINFO *psDevInfo)
{
	...
	printk(KERN_INFO DRIVER_PREFIX
	       ": %s: %d Device %u: Unknown FB format\n",
...
static XBLFB_DEVINFO *XBLFBInitDev(unsigned uiFBDevID)
{
	...
	*/
	if (psDevInfo->sPVRJTable.pfnPVRSRVRegisterCmdProcList(
	    ...
	    aui32SyncCountList, XBLFB_COMMAND_COUNT) != PVRSRV_OK) {
	printk(KERN_ERR DRIVER_PREFIX
...
XBLFB_ERROR XBLFBInit(void)
{
	...
	if (XBLFBGetLibFuncAddr("PVRGetDisplayClassJTable",
	&gpfnGetPVRJTable) != XBLFB_OK) {
...
static XBLFB_BOOL XBLFBDeInitDev(XBLFB_DEVINFO *psDevInfo)
{
	...
	if (psPVRJTable->pfnPVRSRVRemoveCmdProcList(
	    psDevInfo->uiPVRDevID, XBLFB_COMMAND_COUNT) != PVRSRV_OK) {
	printk(KERN_ERR DRIVER_PREFIX
