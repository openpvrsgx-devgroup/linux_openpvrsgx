...
MTKLFB_ERROR MTKLFBGetLibFuncAddr(char *szFunctionName,
...
void MTKLFBQueueBufferForSwap(MTKLFB_SWAPCHAIN *psSwapChain,
...
MTKLFB_BOOL MTKLFBSetUpdateMode(MTKLFB_DEVINFO *psDevInfo,
	MTKLFB_UPDATE_MODE eMode)
{
	return MTKLFB_TRUE;
}
	   ...
	   unsigned long event, void *data)
{
	...
	bBlanked = (*(IMG_INT *)psFBEvent->data != 0) ? MTKLFB_TRUE :
	MTKLFB_FALSE;
...
static void MTKLFBEarlySuspendHandler(struct early_suspend *h)
{
	...
	if (psDevInfo != NULL) {
	MTKLFBAtomicBoolSet(&psDevInfo->sEarlySuspendFlag,
	    MTKLFB_TRUE);
...
static MTKLFB_DEVINFO *MTKLFBPVRDevIDToDevInfo(unsigned uiPVRDevID)
{
	...
	xlog_printk(ANDROID_LOG_ERROR, DRIVER_PREFIX,
	    DRIVER_PREFIX ": %s: PVR Device %u: Couldn't find device\n",
...
#endif
{
	...
	xlog_printk(ANDROID_LOG_ERROR, DRIVER_PREFIX,
	    DRIVER_PREFIX ": %s: MTKLFBInit failed\n",
	    __FUNCTION__);
	return -ENODEV;
	}
...
#endif
{
	...
	xlog_printk(ANDROID_LOG_ERROR, DRIVER_PREFIX,
	    DRIVER_PREFIX ": %s: MTKLFBDeInit failed\n",
	    __FUNCTION__);
	}
